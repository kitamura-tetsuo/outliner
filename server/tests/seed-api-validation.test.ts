...
describe("Seed API Validation", () => {
    ...
    it("should return 400 when projectName is too long (max 255)", async () => {
        ...
        const res = await request(app)
            ...
            .send({
                ...
                pages: [{ name: "Test Page", lines: [] }],
            });

        expect(res.status).to.equal(400);
        expect(res.body.error).to.equal("Invalid request body");
        // Verify that the error is specifically about the max length if possible,
        // or just that it failed validation. Zod returns details.
        // We assume 'projectName' key exists in error details.
        expect(res.body.details.projectName).to.exist;
    });

    it("should return 400 when pages array is too long (max 50)", async () => {
        const manyPages = Array(51).fill({ name: "Page", lines: [] });
        const res = await request(app)
            .post("/api/seed")
            .set("Authorization", "Bearer valid-token")
            .send({
    ...
    it("should return 400 when page name is too long (max 100)", async () => {
        const longPageName = "a".repeat(101);
        const res = await request(app)
            .post("/api/seed")
            .set("Authorization", "Bearer valid-token")
            .send({
                projectName: "valid-project",
                pages: [{ name: longPageName, lines: [] }],
            });
    ...
    it("should return 400 when lines array is too long (max 1000)", async () => {
        const manyLines = Array(1001).fill("line");
        const res = await request(app)
            .post("/api/seed")
            .set("Authorization", "Bearer valid-token")
            .send({
                projectName: "valid-project",
                pages: [{ name: "Page", lines: manyLines }],
            });
    ...
    it("should return 400 when line content is too long (max 10000)", async () => {
        ...
        const res = await request(app)
            ...
            .send({
                projectName: "valid-project",
                pages: [{ name: "Page", lines: [longLine] }],
            });
