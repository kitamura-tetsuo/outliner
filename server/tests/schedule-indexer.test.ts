...
import { computeNextRunAt, handleStoreDocumentForSchedules, initializeScheduleIndex } from "../src/scheduler/schedule-indexer.js";
...
describe("Schedule Indexer", () => {
    ...
    describe("handleStoreDocumentForSchedules", () => {
        ...
        it("should delete a schedule that was removed", () => {
            // First insert
            db.prepare("INSERT INTO schedule_index (room, rule_id, target_table_id, timezone, rrule, dtstart, next_run_at, occurrence_seq, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
                .run("test-room", "rule-to-delete", "table-1", "UTC", "FREQ=DAILY", "2024-01-01T10:00:00", "2024-01-01T10:00:00Z", 0, "active");
        ...
        it("should write back completedAt on exhausted rule", () => {
             ...
             db.prepare("INSERT INTO schedule_index (room, rule_id, target_table_id, timezone, rrule, dtstart, next_run_at, occurrence_seq, state) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
                .run("test-room", "rule-1", "table-1", "UTC", "FREQ=DAILY;COUNT=1", "2024-01-01T10:00:00", null, 1, "disabled");
