...
describe('Firebase emulator起動待機機能 (API-0002)', function () {
        ...
        waitForFirebaseEmulator = async function (maxRetries = 30, initialDelay = 1000, maxDelay = 10000) {
            ...
            while (retryCount < maxRetries) {
                try {
                    ...
                    loggerStub.info(`Firebase emulator connection successful. Found users: ${listUsersResult.users.length}`);
                    if (listUsersResult.users.length > 0) {
                        loggerStub.info(`First user: ${JSON.stringify({
                    ...
                    if (error.code === 'ECONNREFUSED' || error.message.includes('ECONNREFUSED')) {
                        loggerStub.warn(`Firebase emulator not ready yet (attempt ${retryCount}/${maxRetries}): ${error.message}`);
                        ...
                        loggerStub.error(`Firebase emulator connection failed with non-connection error: ${error.message}`);
                        throw error;
                    }
                }
            }
    ...
    it('Firebase emulator環境変数が設定されている場合、emulatorの起動を待機する', async function () {
        ...
        expect(loggerStub.info.calledWith('Firebase emulator detected, waiting for connection... (max retries: 3)')).to.be.true;
    ...
    it('ECONNREFUSED エラーが発生した場合、指数バックオフでリトライする', async function () {
        ...
        expect(loggerStub.warn.calledWith('Firebase emulator not ready yet (attempt 1/3): connect ECONNREFUSED 127.0.0.1:9099')).to.be.true;
        expect(loggerStub.warn.calledWith('Firebase emulator not ready yet (attempt 2/3): connect ECONNREFUSED 127.0.0.1:9099')).to.be.true;
    ...
    it('Firebase Auth emulatorへの接続テストを行い、成功するまで待機する', async function () {
        ...
        expect(loggerStub.info.calledWith('First user: {"uid":"test-uid","email":"test@example.com","displayName":"Test User"}')).to.be.true;
    });
    ...
    it('ECONNREFUSED以外のエラーは即座に失敗とする', async function () {
        ...
        try {
            ...
            expect(loggerStub.error.calledWith('Firebase emulator connection failed with non-connection error: Some other error')).to.be.true;
    ...
    it('接続成功時にユーザー数とユーザー情報をログに出力する', async function () {
        ...
        expect(loggerStub.info.calledWith('Firebase emulator connection successful. Found users: 1')).to.be.true;
        expect(loggerStub.info.calledWith('First user: {"uid":"test-uid-123","email":"user@test.com","displayName":"Test Display Name"}')).to.be.true;
    });
    ...
    it('Firebase初期化が非同期でラップされ、テストユーザーセットアップがemulator起動待機後に実行される', async function () {
        ...
        expect(loggerStub.info.calledWith('開発環境用テストユーザーをセットアップしました: test@example.com (test-user-uid)')).to.be.true;
    });
});
