        ...
        pages: [
            ...
            },
            {
                ...
                lines: [
                    ...
                    "Grid column visibility is explicit at the MCP boundary: every represented physical, computed, or joined result column has shown: true when rendered and shown: false when intentionally hidden, so clients never need to infer visibility from the persisted hidden setting or a separate hidden-column list.",
                    ...
                    "With approved write access, create_grid creates a Grid over an existing Table and appends it to a Page. Preview with dryRun, then apply with the same operationId; use the returned Grid and placement IDs to inspect the result. Retries return those same IDs.",
                    ...
                    "update_table_schema migrates a Table's schema through that same validator and requires acknowledgeDestructive for a column removal or retype; update_table_records writes bounded batches of records by stable record ID, atomically and never by displayed row position, so populating a new ordering column never doubles as a promise about Grid display order.",
                    "Every tool declares the OAuth scope(s) it needs (outliner.read for reads, outliner.read plus outliner.write for a mutation); a connector holding only outliner.read gets a standards-shaped insufficient_scope challenge from a mutation attempt instead of an opaque failure, so ChatGPT can trigger step-up authorization for the missing write scope on its own.",
            ...
            },
            {
                ...
                lines: [
                    ...
                    "Selecting a block directly: on [Advanced Features], press the narrow gutter down a Grid's left edge to select the whole block, or drag from it into the text around it. The block keeps its own clicks and cell selections; Shift+Down and Shift+Up cross it one edge at a time.",
                ...
                items: [
                    ...
                    },
                    {
                        ...
                        children: [
                            {
                                text:
                                    "The Layout is a fixed 12-column grid. Each block inside it stores only how many columns it spans (1 to 12); rows and columns follow from that span plus the block's position in the outline.",
                            },
                            {
                                text:
                                    "Nothing else about placement is saved — no coordinates, row numbers or pixel widths — so reordering the blocks is an ordinary tree reorder and the arrangement follows.",
                            },
                            {
                                text:
                                    "Only visual blocks (Database and Calendar) may sit directly inside a Layout. Ordinary text and a Layout inside a Layout are refused, so text editing stays one-dimensional.",
                            },
                            {
                                text:
                                    "Drag the handle at a block's right edge to change its span a whole column at a time, or use the − / + buttons; the same control takes the arrow keys from the keyboard.",
                            },
                            {
                                text:
                                    "When the Layout gets too narrow for side-by-side reading, the blocks stack one per row in outline order — the stored spans stay as they are and come back with the width.",
                            },
                            {
                                text:
                                    'Right-click the Layout for "Remove layout (keep blocks)": the blocks move up to the Layout\'s position and the container alone is removed, unlike "Delete item", which takes the whole subtree.',
                            },
                        ],
                    },
                    {
