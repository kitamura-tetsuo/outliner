...
export async function initializeFirebase() {
    try {
        ...
        if (!isEmulator && !(serviceAccount && Object.keys(serviceAccount).length > 0 && serviceAccount.private_key)) {
            ...
            const secretsToLoad = ["FIREBASE_PRIVATE_KEY", "FIREBASE_PROJECT_ID", "FIREBASE_CLIENT_EMAIL"];
            await secretManager.loadSecrets(secretsToLoad);

            // Populate process.env from Secret Manager cache
            for (const secretName of secretsToLoad) {
                const value = secretManager.getSecret(secretName);
                if (value) {
