...
export function seedDemoTableDoc(doc: Y.Doc, template: DemoTableTemplate): void {
    ...
    for (const [column, typeOrConfig] of Object.entries(template.components)) {
        const cfg = new Y.Map<unknown>();
        components.set(column, cfg);
        if (typeof typeOrConfig === "string") {
            cfg.set("type", typeOrConfig);
        } else {
            cfg.set("type", typeOrConfig.type);
            cfg.set("label", typeOrConfig.label);
        }
    }
