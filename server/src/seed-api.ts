...
export function createSeedRouter(
    ...
    router.post("/seed", async (req, res): Promise<void> => {
        ...
        try {
            ...
            const directConnection = await (hocuspocus as unknown as { openDirectConnection: (room: string, opts: unknown) => Promise<{ document: import("yjs").Doc }> }).openDirectConnection(projectRoom, {
                isSeeding: true,
            });
