    ...
    const hocuspocus = new Hocuspocus(
        {
            ...
            async onConnect(data: import("@hocuspocus/server").onConnectPayload<ConnectionContext>) {
                const ip = data.context?.ip || (data.requestHeaders as Record<string, string | undefined>)?.["x-forwarded-for"]
    ...
    server.on("upgrade", (request: any, socket: any, head: any) => {
        wss.handleUpgrade(request, socket, head, (ws: any) => {
            ...
            try {
                ...
                clientConnection = hocuspocus.handleConnection(ws, webRequest, { ip } as unknown as import("@hocuspocus/server").ConnectionContext); // Partial context initially
                ...
                ws.on("message", (data: any) => {
                    ...
                    if (len > config.MAX_MESSAGE_SIZE_BYTES) {
                        ...
                        try {
                            ...
                            // Send stateless JSON error so the client can handle the size limit gracefully.
                            if (clientConnection) {
