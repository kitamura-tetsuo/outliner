        ...
        app.get("/api/telemetry-logs", (req, res): any => {
            try {
                ...
                fs.open(telemetryLogPath, "r", (err, fd) => {
                    if (err) {
                        logger.error(
                            { error: new Error(`Failed to open Telemetry log file: ${err.message}`) },
                            `Failed to open Telemetry log file: ${err.message}`,
                        );
                    ...
                    fs.read(fd, buffer, 0, length, position, (err, bytesRead, buffer) => {
                        ...
                        if (err) {
                            logger.error(
                                { error: new Error(`Failed to read Telemetry log file: ${err.message}`) },
                                `Failed to read Telemetry log file: ${err.message}`,
                            );
