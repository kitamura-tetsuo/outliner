...
// aliasPickerStore
content = content.replace(
    /const ap: any = W\.aliasPickerStore;/g,
...
content = content.replace(
    /const ap:any = \(window as Window.*?\)\.aliasPickerStore;/g,
...
content = content.replace(
    /const gs: any = generalStore as any;/g,
...
content = content.replace(
    /const items: any = cp\?\.items as any;/g,
    "const items = cp?.items as unknown as { iterateUnordered?: () => Iterable<unknown> };",
);

content = content.replace(
    /\(generalStore as any\)\.openCommentItemId/g,
    "(generalStore as unknown as { openCommentItemId: string | null }).openCommentItemId",
);
content = content.replace(
    /\(generalStore as any\)\.openCommentItemIndex/g,
    "(generalStore as unknown as { openCommentItemIndex: number | null }).openCommentItemIndex",
);

content = content.replace(
    /function normalizeCommentCount\(arr: any\): number/g,
...
content = content.replace(
    /function applyCommentCount\(arrOrCount: any\)/g,
...
content = content.replace(
    /const ap: any = aliasPickerStore as any;/g,
    "const ap = aliasPickerStore as unknown as { openAliasSearch: (...args: unknown[]) => void };",
);
content = content.replace(
    /void \(aliasPickerStore as any\)\?\.tick;/g,
    "void (aliasPickerStore as unknown as { tick?: unknown })?.tick;",
);

content = content.replace(
    /const lastItemId = \(aliasPickerStore as any\)\?\.lastConfirmedItemId;/g,
    "const lastItemId = (aliasPickerStore as unknown as { lastConfirmedItemId?: string })?.lastConfirmedItemId;",
);
content = content.replace(
    /const lastTargetId = \(aliasPickerStore as any\)\?\.lastConfirmedTargetId;/g,
    "const lastTargetId = (aliasPickerStore as unknown as { lastConfirmedTargetId?: string })?.lastConfirmedTargetId;",
);
content = content.replace(
    /const lastAt = \(aliasPickerStore as any\)\?\.lastConfirmedAt as number \| null;/g,
    "const lastAt = (aliasPickerStore as unknown as { lastConfirmedAt?: number | null })?.lastConfirmedAt;",
);

content = content.replace(
    /const targetEl: any = \(ev\?\.target as any\)\?\.closest\?\.\('\.outliner-item'\) \|\| null;/g,
    'const targetEl = (ev?.target as Element | null)?.closest?.(".outliner-item") || null;',
);

content = content.replace(
    /try \{ \(model\.original as any\)\.attachments\.push\(\[url\]\); \} catch \{\}/g,
    "try { (model.original as unknown as { attachments: string[][] }).attachments.push([url]); } catch {}",
);
content = content.replace(
    /const w: any = \(typeof window !== 'undefined'\) \? \(window as Window.*?\)/g,
    'const w = (typeof window !== "undefined") ? (window as Window & typeof globalThis & { generalStore?: any })',
);
content = content.replace(
    /const curPage: any = w\?\.generalStore\?\.currentPage;/g,
    "const curPage = w?.generalStore?.currentPage as { items?: { at?: (i: number) => any } } | undefined;",
);
content = content.replace(
    /try \{ \(cand as any\)\.attachments\.push\(\[url\]\); \} catch \{\}/g,
    "try { (cand as unknown as { attachments: string[][] }).attachments.push([url]); } catch {}",
);

content = content.replace(
    /const setMapField = \(it: any, key: string, value: any\) => \{/g,
    "const setMapField = (it: unknown, key: string, value: unknown) => {",
);
content = content.replace(
    /if \("componentType" in \(item as any\)\) \{/g,
