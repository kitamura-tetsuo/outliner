...
function extractPagePreview(pageItem: Item, maxLines: number = 3, maxDepth: number = 3) {
    ...
    function traverse(item: Item, currentDepth: number) {
        ...
        try {
            if (item.items) {
                ...
                // use iterable length manually
                let len = 0;
                for (const _ of children) len++;
                // console.log("item.items length iterable:", len, "property length:", children.length);
    ...
    try {
        if (pageItem.items) {
            ...
            let len = 0;
            for (const _ of rootChildren) len++;
            console.log("rootChildren length iterable:", len, "property length:", rootChildren.length);

            for (let k = 0; k < len; k++) {
                if (i++ > 20) break;
                const child = rootChildren.at(k);
                console.log("k:", k, "child:", child ? child.text : "undefined");
