...
// DragEvent wrap
content = content.replace(
    /function\(this: any, orig: any, event: Event\): boolean/g,
    "function(this: unknown, orig: (...args: unknown[]) => unknown, event: Event): boolean",
);
content = content.replace(
    /\(event as any\)\.dataTransfer/g,
    "(event as unknown as { dataTransfer: DataTransfer }).dataTransfer",
);
...
content = content.replace(
    /const anyWin: any = window as Window.*?;\n/g,
...
content = content.replace(
    /const itemsProto = \(DataTransferItemList as any\)\?\.prototype;/g,
