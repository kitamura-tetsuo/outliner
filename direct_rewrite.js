...
function applyGantt() {
    ...
    | {
        ...
        if (line.includes("onLeafResizeMove?.(")) {
            out.push(line);
            out.push(
                "            pointerTracker.label = formatDragResizeLabel(entry as any, newDurationMs, timeZone);",
            );
            continue;
        }
        ...
        if (line.includes("// Shift previews are not currently sent up")) {
            out.push(line);
            out.push(
                "            pointerTracker.label = formatSubtreeShiftLabel(snappedDeltaMs, pointerTracker.originalMs + snappedDeltaMs, timeZone);",
            );
            continue;
        }
...
function applyView() {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (
            line.includes("<CalendarGanttChart") || line.includes("<CalendarMonthGrid")
            || line.includes("<CalendarLaneTimeGrid") || line.includes("<CalendarTimeGrid")
        ) {
            out.push(line);
            out.push("            timeZone={timeZone}");
            continue;
