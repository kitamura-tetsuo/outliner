...
type DocumentWithCaret = Document & {
    caretPositionFromPoint?: (x: number, y: number, options?: unknown) => LocalCaretPosition | null;
...
describe("getClickPosition", () => {
    ...
    it("should use caretPositionFromPoint if available and valid", () => {
        ...
        doc.caretPositionFromPoint = vi.fn((x: number, y: number) => {
            if (x === 15 && y === 0) {
                return {
                    offsetNode: mockNode,
                    offset: 3,
                    getClientRect: () => new DOMRect(),
                } as unknown as LocalCaretPosition;
            }
            return null;
        });
        ...
        delete (doc as unknown as Record<string, unknown>).caretPositionFromPoint;
        delete (doc as unknown as Record<string, unknown>).createRange;
        document.body.removeChild(div);
    });
    ...
    it("should use caretRangeFromPoint if available and valid", () => {
        ...
        delete (doc as unknown as Record<string, unknown>).caretRangeFromPoint;
        document.body.removeChild(div);
    });
    ...
    it("should fallback gracefully if doc.caretRangeFromPoint is undefined", () => {
        ...
        (doc as unknown as Record<string, unknown>).caretRangeFromPoint = undefined;
        (doc as unknown as Record<string, unknown>).caretPositionFromPoint = undefined;
