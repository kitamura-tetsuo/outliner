...
class PollingMonitor {
    ...
    start() {
        ...
        // setIntervalをインターセプト
        window.setInterval = (callback: unknown, delay?: number, ...args: unknown[]): unknown => {
            ...
            // タイマーIDをマッピング
            (call as unknown).timerId = timerId;

            return timerId;
        };

        // setTimeoutをインターセプト
        window.setTimeout = (callback: unknown, delay?: number, ...args: unknown[]): unknown => {
            const stack = new Error().stack || "";
            const id = this.nextId++;
        ...
        window.requestAnimationFrame = (callback: FrameRequestCallback): number => {
            ...
            (call as unknown).frameId = frameId;
...
if (typeof window !== "undefined") {
    window.__pollingMonitor = pollingMonitor;
}
