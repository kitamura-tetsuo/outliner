...
export function saveProjectSnapshot(project: Project | undefined): void {
    ...
    try {
        const snapshot: ProjectSnapshot = {
            title: project.title ?? "",
            items: collectItems(project.items as unknown),
        };
...
export function snapshotToProject(snapshot: ProjectSnapshot): Project {
    ...
    for (const root of snapshot.items) {
        ...
        populateChildren(page.items as unknown, root.children);
    }
...
function populateChildren(items: unknown, children: SnapshotItem[]) {
    ...
    for (const child of children) {
        ...
        populateChildren(node.items as unknown, child.children);
    }
}
...
export function createSnapshotClient(projectName: string, project: Project): unknown {
    ...
    try {
        if (typeof window !== "undefined") {
            const registry = window.__YJS_CLIENT_REGISTRY__;
            if (registry?.set) {
...
function projectToAllData(project: Project): unknown {
    ...
    return { items: walk(project.items as unknown) };
}
