...
import type { IConfigProviderBase, ConfigTypes } from "@fluidframework/core-interfaces";
...
export class TelemetryFilterLogger {
    ...
    private isTelemetryLog(message: string): boolean {
        ...
        return this.telemetryKeywords.some(keyword =>
    ...
    // 標準的なログレベルメソッドを実装
    trace(...args: any[]): void { this.send("trace", ...args); }
    debug(...args: any[]): void { this.send("debug", ...args); }
    info(...args: any[]): void { this.send("info", ...args); }
    warn(...args: any[]): void { this.send("warn", ...args); }
    error(...args: any[]): void { this.send("error", ...args); }
    fatal(...args: any[]): void { this.send("fatal", ...args); }
}
