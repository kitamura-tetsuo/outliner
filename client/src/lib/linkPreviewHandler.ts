...
function createPreviewContent(pageName: string, projectName?: string): HTMLElement {
    ...
    Object.entries(PREVIEW_STYLES).forEach(([key, value]) => {
        previewElement.style[key as unknown] = value;
    });
    ...
    if (foundPage) {
        ...
        if (foundPage.items && (foundPage.items as unknown).length > 0) {
            const ul = document.createElement("ul");
            ul.style.margin = "0";
            ul.style.padding = "0 0 0 20px";

            // 最大5つまでのアイテムを表示
            const maxItems = Math.min(5, (foundPage.items as unknown).length);
            for (let i = 0; i < maxItems; i++) {
                const item = (foundPage.items as unknown)[i];
                if (item) {
            ...
            // 5つ以上ある場合は「...」を表示
            if ((foundPage.items as unknown).length > 5) {
