...
export class KeyEventHandler {
    ...
    private static initKeyHandlers() {
        ...
        add("Escape", false, false, false, () => {
            // If alias picker is visible, close it
    ...
    /**
     * Delegate KeyDown event to each cursor
     */
    static handleKeyDown(event: KeyboardEvent) {
        // Generally ignore events handled by other handlers. Continue for Enter/Arrow/Escape if palette is visible.
        if ((event as KeyboardEvent).defaultPrevented) {
            if (!commandPaletteStore.isVisible) return;
        }
        // Alias picker visible: Forward Arrow/Enter/Escape to picker
        try {
            if (aliasPickerStore.isVisible) {
                ...
                if (key === "Enter") {
                    try {
                        // Confirm directly from store selected index first (independent of DOM)
                        try {
                        ...
                        // To ensure stable confirmation even without selection state,
                        // Use the second item in list (or first if not exists) by default
                        ...
                        // Set the first content line (= secondId in tests) as target
                        try {
                            const w: any = window as any;
                            ...
                            if (root && aliasId && firstContent?.id) {
                                ...
                                try {
                                    ...
                                    // Set fallback for the last item in the model as well
        ...
        // Pre-processing: Ensure command palette is shown on slash input
        if (event.key === "/") {
            try {
                ...
                if (!preventPalette) {
                    ...
                    // Let Slash input be processed normally (accumulate query in subsequent Input)
                }
            } catch (e) {
        ...
        if (commandPaletteStore.isVisible) {
            if (event.key === "ArrowDown") {
                ...
                // Palette visible: Always prioritize confirming Alias if included in filter
                try {
                ...
                // Process directly if text immediately before is "/alias"
                try {
                    const cursor = cursorInstances[0];
                    const node = cursor.findTarget();
                    const text = node?.text || "";
        ...
        // Auxiliary: Support alias creation on Enter immediately after '/alias' when palette is hidden
        if (event.key === "Enter" && !commandPaletteStore.isVisible) {
            try {
                ...
                // Strict detection using textarea actual value as well
                const gs: any = typeof window !== "undefined" ? (window as any).generalStore : null;
                ...
                if (aliasDetected) {
                    ...
                    if (items && typeof items.addNode === "function") {
                        ...
                        if (newItem) {
                            ...
                            return; // Complete processing here
                        }
                    }
                }
            } catch (e) {
        ...
        // Call onKeyDown method of each cursor instance
        let handled = false;
        ...
        if (handled) {
            ...
            // Follow-up processing to open AliasPicker after normal processing (cursor.onKeyDown etc)
            if (shouldOpenAliasPickerAfterDefault) {
                try {
                    setTimeout(() => {
                        try {
    ...
    /**
     * Delegate Input event to each cursor
     */
    static handleInput(event: Event) {
        ...
        // Buffer recent input stream (for palette fallback detection)
        try {
            const w: any = typeof window !== "undefined" ? (window as any) : null;
        ...
        if (inputEvent.data === "/") {
            // Check character before cursor position to determine if part of internal link
            if (cursorInstances.length > 0) {
                const cursor = cursorInstances[0];
                const node = cursor.findTarget();
            ...
            // Use dedicated input processing if CommandPalette is displayed
            if (inputEvent.data) {
        ...
        if (textareaElement) {
            ...
            // Error log if textarea not found
            if (typeof window !== "undefined" && (window as any).DEBUG_MODE) {
    ...
    // Keep current composition character count
    static lastCompositionLength = 0;

    /**
     * Handle IME compositionstart event
     */
    ...
    /**
     * Handle IME compositionupdate event and display intermediate input characters
     */
    static handleCompositionUpdate(event: CompositionEvent) {
        const data = event.data || "";
        const cursorInstances = store.getCursorInstances();
        // Delete previous intermediate characters
    ...
    /**
     * Handle IME compositionend event and insert Japanese input
     */
    static handleCompositionEnd(event: CompositionEvent) {
        const data = event.data || "";
        const cursorInstances = store.getCursorInstances();
        // Delete intermediate characters
        if (KeyEventHandler.lastCompositionLength > 0) {
            cursorInstances.forEach(cursor => {
                for (let i = 0; i < KeyEventHandler.lastCompositionLength; i++) {
                    cursor.deleteBackward();
    ...
    static handleCopy(event: ClipboardEvent) {
        ...
        if (selectedText) {
            try {
                ...
                // Not used in production, but required to verify clipboard content in E2E tests
    ...
    static handleBoxSelection(event: KeyboardEvent) {
        ...
        // Start box selection if not already started
        if (!KeyEventHandler.boxSelectionState.active) {
            ...
            // Set initial state of box selection (show selection-box at start)
            ...
            // isUpdating flag is managed by EditorOverlayStore.setBoxSelection,
            // DOM manipulation is not needed here
        ...
        if (
            ...
            try {
                ...
                if (typeof window !== "undefined") {
                    // Show hint indicating direction
    ...
    /**
     * 2つのアイテム間のすべてのGet itemする
     * @param startItemId 開始Item ID
     * @param endItemId 終了Item ID
    ...
    /**
     * Async method to handle paste event.
     * Caller should `await` to catch permission denial or read failure.
     * Catch Clipboard API permission error and log in DEBUG_MODE.
     * On failure, dispatch `clipboard-permission-denied` or `clipboard-read-error`
     * and insert empty string to make it look like nothing was pasted to user.
     * @param event ClipboardEvent
     */
    static async handlePaste(event: ClipboardEvent): Promise<void> {
        ...
        try {
            ...
            // Use Clipboard API if cannot get from event
            ...
            // Get from global variable if text cannot be obtained (test environment fallback only)
            // This path is not executed in production as event.clipboardData is always available
            // This fallback is needed as Clipboard API might be restricted in E2E test environment
            ...
            // Save to global variable (E2E test environment only)
            // Not used in production, but required to verify paste content in E2E tests
            if (typeof window !== "undefined") {
            ...
            if (
                ...
                // pasteMode: 'spread' - Insert different text to each cursor
                // pasteMode: 'full' - Insert same text to each cursor
                if (pasteMode === "spread") {
                    // Insert corresponding text to each cursor
                    cursorInstances.forEach((cursor, index) => {
                        if (index < multicursorText.length) {
                            cursor.insertText(multicursorText[index]);
                        } else if (multicursorText.length > 0) {
                            // Use last text repeatedly if more cursors than texts
                            cursor.insertText(multicursorText[multicursorText.length - 1]);
                        }
                    });
                } else {
                    // 'full'モード: Insert same text to each cursor
            ...
            if (
                ...
                // Insert line corresponding to each cursor
                const cursorInstances = store.getCursorInstances();
                cursorInstances.forEach((cursor, index) => {
                    if (index < lines.length) {
                        cursor.insertText(lines[index]);
                    } else if (lines.length > 0) {
                        // Use last line repeatedly if more cursors than lines
            ...
            // Consider multi-item paste if normal multi-line text
            if (text.includes("\n")) {
                const lines = text.split(/\r?\n/);
                ...
                // Insert first line to each cursor if multiple cursors
                // Insert only first line if single cursor
            ...
            // Insert at cursor position if single line text
            const cursorInstances = store.getCursorInstances();
            cursorInstances.forEach(cursor => cursor.insertText(text));
        } catch (error) {
            // Log if error occursし UI に通知
            if (typeof window !== "undefined" && (window as any).DEBUG_MODE) {
    ...
    static handleCut(event: ClipboardEvent) {
        ...
        // Delete selected text (essence of cut operation)
        if (selectedText) {
            const cursorInstances = store.getCursorInstances();
            cursorInstances.forEach(cursor => {
                // Delete selection (cut operation)
