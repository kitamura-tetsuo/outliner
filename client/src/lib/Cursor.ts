...
export class Cursor implements CursorEditingContext {
    ...
    private navigateToItem(direction: "left" | "right" | "up" | "down") {
        ...
        if (direction === "left") {
            ...
            const isParentItem = parentOfCurrent && prevItem && "id" in parentOfCurrent
                && prevItem.id === (parentOfCurrent as unknown as { id: string; }).id;
                        ...
                        if (nextItemElement) {
                            ...
                            if (nextItemId && nextItemId !== this.itemId) {
                                ...
                                if (
                                    ...
                                    && (window as unknown as { DEBUG_MODE?: boolean; }).DEBUG_MODE
                                ) {
                                    console.log(
    ...
    private collectAllItemIds(node: any, ids: string[]): string[] {
        ...
        if (
            node.items
            && typeof (node.items as unknown as { [Symbol.iterator]?: () => Iterator<any>; })[Symbol.iterator]
                === "function"
        ) {
