...
export function collectBacklinks(targetPageName: string): Backlink[] {
    ...
    try {
        ...
        // Optimization: Iterate directly to avoid O(N log N) sorting caused by [Symbol.iterator]
        // (which calls childrenKeys() and sorts them in app-schema.ts)
        const pageIterator =
            (typeof (pages as Iterable<Item> & { iterateUnordered?: () => Iterator<Item>; }).iterateUnordered
                    === "function")
                ? (pages as Iterable<Item> & { iterateUnordered?: () => Iterator<Item>; }).iterateUnordered!()
                : pages;

        for (const page of pageIterator) {
