 ...
 *     const m = window.__DOC__.getMap("m");
 *
...
export async function initializeBrowserPage(
    ...
    if (requireAuth) {
        await page.evaluate(async () => {
            const mgr = window.__USER_MANAGER__;
        ...
        await page.waitForFunction(
            () => !!window.__USER_MANAGER__?.getCurrentUser?.(),
            null,
            { timeout: 10000 },
        );
    }
...
export async function createMinimalYjsConnection(
    ...
    return await page.evaluate(
        async ({ pid, docVar, providerVar, enableLogging, importPath }) => {
            ...
            window[docVar] = conn.doc;
            const provider = conn.provider as unknown;
            window[providerVar] = provider;

            if (enableLogging) {
                provider.on("status", (e: unknown) => console.log(`[${providerVar}] status`, e.status));
 ...
 * });
 * const updateCount = await page.evaluate(() => window.__UPDATES2__);
 * ```
 */
...
export async function prepareTwoFullBrowserPages(
    ...
    await page1.waitForFunction(
        () => {
            const yjsStore = window.__YJS_STORE__;
    ...
    await page2.waitForFunction(
        () => {
            return !!window.__USER_MANAGER__;
        },
        { timeout: 10000 },
    );
