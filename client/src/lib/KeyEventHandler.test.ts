...
describe("KeyEventHandler.handlePaste", () => {
    ...
    beforeEach(() => {
        vi.spyOn(document, "activeElement", "get").mockReturnValue({ classList: { contains: (cls: string) => cls === "global-textarea" }, closest: () => null } as unknown as Element);
        vi.clearAllMocks();
    ...
    it("dispatches read error when cursor insertion throws", async () => {
        ...
        const mockFn = mockInsertText as import("vitest").Mock;
