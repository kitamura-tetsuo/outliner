...
export class CursorBase {
    ...
    constructor(cursorId: string, opts: { itemId: string; offset: number; isActive: boolean; userId: string; }) {
    ...
    insertLineBreak() {
        ...
        // タイトルかどうかを判断
        const isPageTitle = this.isPageItem(target);
    ...
    deleteMultiItemSelection(selection: any) {
        ...
        // 選択範囲の開始と終了アイテムを取得
        const startItem = searchItem(generalStore.currentPage!, selection.startItemId);
        const endItem = searchItem(generalStore.currentPage!, selection.endItemId);
