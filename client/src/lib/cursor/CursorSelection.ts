...
export class CursorSelection {
    ...
    extendSelectionLeft() {
        ...
        if (existingSelection) {
            ...
            if (existingSelection.isReversed) {
                ...
                // Reverse the direction if the selection disappears
        ...
        // Clear existing selections for the same user before setting a new range
        store.clearSelectionForUser(this.cursor.userId);
        store.setSelection({
            startItemId,
            startOffset,
    ...
    extendSelectionDown() {
        ...
        if (existingSelection) {
            ...
            // If the destination is within the same item, select the entire text
            if (this.cursor.itemId === oldItemId) {
                ...
                endItemId = this.cursor.itemId;
                endOffset = this.cursor.offset; // Select up to the current position in the next item (previously fixed at 0)
                isReversed = false;
        ...
        // Properly set the direction of the selection (removed forced setting for testing)
        // If start and end are the same item, decide direction by offset
        if (startItemId === endItemId) {
            isReversed = startOffset > endOffset;
        } // If different items, decide direction by order in DOM
        else {
        ...
        // Wait a little for the DOM to reflect to confirm that the selection was created correctly
        if (typeof window !== "undefined") {
            setTimeout(() => {
                ...
                // If the selection is not displayed, set the selection again
                if (selectionElements.length === 0) {
                    ...
                    // Force update the display of the selection
                    store.forceUpdate();
                }
    ...
    moveToLineStart() {
        ...
        // Confirm that the cursor has been updated correctly
    ...
    extendSelectionToLineStart() {
        ...
        if (existingSelection) {
            ...
            if (this.cursor.offset > lineStartOffset) {
                ...
                // If cursor is at the start of the line (usually doesn't enter here)
                startOffset = lineStartOffset;
                endOffset = this.cursor.offset;
                isReversed = false;
            }
