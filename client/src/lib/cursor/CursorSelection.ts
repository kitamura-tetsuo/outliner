...
interface CursorContext {
    ...
    itemId: string;
    offset: number;
    findTarget(): Item | undefined | null;
    moveLeft(): void;
    moveRight(): void;
    moveUp(): void;
    moveDown(): void;
    updateGlobalTextareaSelection(
        startItemId: string | undefined,
        startOffset: number | undefined,
        endItemId: string | undefined,
...
export class CursorSelection {
    private cursor: CursorContext; // Holds an instance of the Cursor class

    constructor(cursor: CursorContext) {
        this.cursor = cursor;
    }
    ...
    extendSelectionLeft() {
        ...
        if (existingSelection) {
            ...
            if (existingSelection.isReversed) {
                // Move start position if the selection is reversed
                ...
                // Reverse direction if the selection disappears
                ...
                // Move end position if the selection is forward (not reversed)
        ...
        // Clear existing selection for the same user before setting the new range
        store.clearSelectionForUser(this.cursor.userId);
        store.setSelection({
            startItemId,
            startOffset,
        ...
        // Set the selection range for the global textarea
    ...
    extendSelectionDown() {
        ...
        if (existingSelection) {
            ...
            // Select all text if moving within the same item
            if (this.cursor.itemId === oldItemId) {
                ...
                endItemId = this.cursor.itemId;
                endOffset = this.cursor.offset; // Select up to the current position of the next item (previously fixed at 0)
                isReversed = false;
        ...
        // Set the selection direction appropriately (removed forced setting for tests)
        ...
        // Wait a bit for the DOM to reflect that the selection was created correctly
        if (typeof window !== "undefined") {
            setTimeout(() => {
                ...
                // If the selection is not displayed, set it again
                if (selectionElements.length === 0) {
                    ...
                    // Force update of selection display
                    store.forceUpdate();
                }
    ...
    extendSelectionToLineStart() {
        ...
        // Do nothing if the current cursor position is already at the start of the line
        ...
        if (existingSelection) {
            ...
            // Create a new selection
            // Select from current position to the start of the line
            startItemId = this.cursor.itemId;
            endItemId = this.cursor.itemId;
            ...
            if (this.cursor.offset > lineStartOffset) {
                ...
                startOffset = this.cursor.offset;
                endOffset = lineStartOffset;
                isReversed = true; // Reversed direction as we are selecting towards the start of the line
            } else {
                // Case where cursor is at the start of the line (usually does not enter here)
                startOffset = lineStartOffset;
                endOffset = this.cursor.offset;
                isReversed = false;
            }
        ...
        // Move cursor position to the start of the line
    ...
    extendSelectionToLineEnd() {
        ...
        // Do nothing if the current cursor position is already at the end of the line
