...
export class CursorSelection {
    ...
    extendSelectionLeft() {
        ...
        if (existingSelection) {
            ...
            if (existingSelection.isReversed) {
                ...
                // If forward selection, move end position
        ...
        // Clear existing selection for the same user before setting new one
        store.clearSelectionForUser(this.cursor.userId);
        store.setSelection({
            startItemId,
            startOffset,
    ...
    extendSelectionDown() {
        ...
        if (existingSelection) {
            ...
            // Select all text if destination is within the same item
    ...
    extendSelectionToLineStart() {
        ...
        if (existingSelection) {
            ...
            if (this.cursor.offset > lineStartOffset) {
                ...
                // Case where cursor is at start of line (usually does not enter here)
                startOffset = lineStartOffset;
                endOffset = this.cursor.offset;
                isReversed = false;
            }
