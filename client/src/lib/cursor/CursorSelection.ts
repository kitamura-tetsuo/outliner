...
export class CursorSelection {
    ...
    extendSelectionLeft() {
        ...
        // Clear existing selection for the same user, then set the new range
        store.clearSelectionForUser(this.cursor.userId);
        store.setSelection({
            startItemId,
            startOffset,
        ...
        // Set the selection range for the global text area
    ...
    extendSelectionDown() {
        ...
        if (existingSelection) {
            ...
            // If the move destination is within the same item, select the entire text
            if (this.cursor.itemId === oldItemId) {
                ...
                endItemId = this.cursor.itemId;
                endOffset = this.cursor.offset; // Select up to the current position of the next item (previously was fixed at 0)
                isReversed = false;
        ...
        // Set the direction of the selection appropriately (removed forced setting for testing)
        ...
        if (startItemId === endItemId) {
            ...
            // If index is not found, default to forward
        ...
        // Wait a little for the selection to be reflected in the DOM to ensure it is created correctly
        if (typeof window !== "undefined") {
            ...
            }, 150); // Increased timeout to 150ms to allow more time for DOM updates
        }
    }
    ...
    extendSelectionToLineEnd() {
        ...
        if (typeof window !== "undefined") {
            ...
            }, 100); // Increased timeout to 100ms to allow more time for DOM updates
