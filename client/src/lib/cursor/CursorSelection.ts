...
export class CursorSelection {
    ...
    extendSelectionLeft() {
        ...
        if (existingSelection) {
            ...
            if (existingSelection.isReversed) {
                // Move start position if selection is reversed
                ...
                // Move end position if selection is forward
    ...
    extendSelectionDown() {
        ...
        // Wait a bit for the DOM to reflect to ensure selection was created correctly
        if (typeof window !== "undefined") {
            setTimeout(() => {
                ...
                // Re-set selection if not visible
    ...
    extendSelectionToLineStart() {
        ...
        // Do nothing if cursor is already at the start of the line
        ...
        if (existingSelection) {
            ...
            if (this.cursor.offset > lineStartOffset) {
                ...
                startOffset = this.cursor.offset;
                endOffset = lineStartOffset;
                isReversed = true; // Selection is towards the start, so it is reversed
            } else {
                // If cursor is at the start (should not happen normally)
                startOffset = lineStartOffset;
                endOffset = this.cursor.offset;
                isReversed = false;
            }
    ...
    extendSelectionToLineEnd() {
        ...
        // Do nothing if cursor is already at the end of the line
