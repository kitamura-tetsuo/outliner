...
class GeneralStore {
    ...
    setUserContainer(value: UserContainer | null) {
        ...
        const self = firestoreStore as unknown as GeneralStore;
        ...
        const arrayHandler: ProxyHandler<string[]> = {
            get: (target, prop, receiver) => {
                ...
                if (
                    ...
                    return (...args: unknown[]) => {
                        const res = Function.prototype.apply.call(v, target, args);
                        ...
                        try {
                            ...
                            (firestoreStore as unknown as GeneralStore).setUserContainer(next); // API 経由で ucVersion++ を確実に反映
                        } catch (e) {
...
const existingCandidate = typeof window !== "undefined"
    ? (window as Window & { __FIRESTORE_STORE__?: unknown; }).__FIRESTORE_STORE__
    : (globalThis as unknown as { __FIRESTORE_STORE__?: unknown; }).__FIRESTORE_STORE__;
...
if (typeof window !== "undefined") {
    ...
    if (isTestEnv) {
        (window as Window & { __FIRESTORE_STORE__?: unknown; }).__FIRESTORE_STORE__ = firestoreStore;
    }
}
