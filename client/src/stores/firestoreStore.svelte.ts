...
class GeneralStore {
    ...
    setUserContainer(value: UserContainer | null) {
        ...
        const self = firestoreStore as unknown;
        ...
        const arrayHandler: ProxyHandler<string[]> = {
            get: (target, prop, receiver) => {
                ...
                if (
                    ...
                    return (...args: unknown[]) => {
                        const res = (v as unknown).apply(target, args);
                        ...
                        try {
                            ...
                            (firestoreStore as unknown).setUserContainer(next); // API 経由で ucVersion++ を確実に反映
                        } catch (e) {
...
const existingCandidate = typeof window !== "undefined"
    ...
    : (globalThis as unknown).__FIRESTORE_STORE__;
...
(firestoreStore as unknown).__isRealFirestoreStore = true;
