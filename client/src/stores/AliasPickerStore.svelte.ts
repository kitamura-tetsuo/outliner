...
class AliasPickerStore {
    ...
    confirmById(id: string) {
        ...
        if (!this.itemId) {
            try {
                ...
                const eos: unknown = w?.editorOverlayStore;
                ...
                if (activeId) {
                    ...
                    const items: unknown = (generalStore.currentPage as unknown)?.items;
                    const last = items && typeof items.length === "number" && items.length > 0
                        ? ((items as unknown).at
                            ? (items as unknown).at(items.length - 1)
                            : (items as unknown)[items.length - 1])
                        : null;
                    if (last?.id) {
                        this.itemId = last.id;
                    }
    ...
    private collectOptions(): Option[] {
        ...
        if (root) {
            try {
                const children = (root as unknown).items as Items;
                if (children && typeof children.length === "number") {
                    const rawRootText: unknown = (root as unknown).text;
                    ...
                    for (let i = 0; i < children.length; i++) {
                        ...
                        try {
                            child = (children as unknown).at
                                ? (children as unknown).at(i) as Item
                                : (children as unknown)[i] as Item;
                        } catch {
                            child = undefined;
                        }
                        if (child && child.id) {
    ...
    private traverse(node: Item, path: string[], out: Option[], visited = new Set<string>(), depth = 0) {
        ...
        try {
            ...
            if (!children) return;
            const len = Number.isFinite((children as unknown).length) ? (children as unknown).length as number : 0;
            for (let i = 0; i < len; i++) {
                let child: Item | undefined;
                try {
                    child = (children as unknown).at
        ...
        while (queue.length > 0 && iterations < maxIterations) {
            ...
            if (items) {
                for (let i = 0; i < items.length; i++) {
                    const child = (items as unknown).at
                        ? (items as unknown).at(i) as Item
                        : (items as unknown)[i] as Item;
    ...
    private findItem(node: Item, id: string, depth = 0): Item | undefined {
        ...
        if (items) {
            for (let i = 0; i < items.length; i++) {
                const child = (items as unknown).at ? (items as unknown).at(i) as Item : (items as unknown)[i] as Item;
                if (child) {
