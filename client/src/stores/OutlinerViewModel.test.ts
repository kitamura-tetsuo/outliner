...
describe("OutlinerViewModel", () => {
    ...
    it("toggleCollapsed hides children from visible items", () => {
        ...
        const grand = tree.items.at(0)!.items.addNode("u");
        grand.id = "grand";
        grand.text = "grand";
        vm.updateFromModel(tree);
    ...
    it("tracks comment counts", () => {
        const c = tree.items.at(0)!.addComment("u", "hi");
        vm.updateFromModel(tree);
        expect(vm.getViewModel("child1")?.commentCount).toBe(1);
        tree.items.at(0)!.updateComment(c.id, "edited");
        vm.updateFromModel(tree);
        expect(vm.getViewModel("child1")?.commentCount).toBe(1);
        tree.items.at(0)!.deleteComment(c.id);
        vm.updateFromModel(tree);
