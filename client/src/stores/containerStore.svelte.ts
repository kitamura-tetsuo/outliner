...
if (typeof window !== "undefined") {
    ...
    if (isTestEnv) {
        window.__CONTAINER_STORE__ = containerStore;
    }
}

// Hook into firestoreStore updates to keep containers mirrored without polling
const originalSetUserContainer = (firestoreStore as unknown).setUserContainer?.bind(firestoreStore);
if (typeof originalSetUserContainer === "function") {
    (firestoreStore as unknown).setUserContainer = (value: UserContainer | null) => {
