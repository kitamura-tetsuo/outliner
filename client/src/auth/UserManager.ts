...
export class UserManager {
    ...
    constructor() {
        ...
        if (useEmulator) {
            ...
            try {
                ...
                // エミュレーターに接続できない場合
                logger.error("Failed to connect to Auth emulator", { error: err });
    ...
    private connectToFirebaseEmulator(): boolean {
        try {
            ...
            logger.error("Error connecting to Firebase Auth emulator", { error: err });
            return false;
        }
    }
        ...
        if (isTestEnvironment) {
            ...
            try {
                ...
                try {
                    ...
                    logger.error("[UserManager] Failed to create test user", { error: createError });
                }
            }
            return;
        }
                ...
                try {
                    ...
                    if (errorObj.code === "auth/user-not-found") {
                        try {
                            ...
                            logger.error("[UserManager] Failed to create new user", { error: createError });
                        }
                    }
    ...
    public async refreshToken(): Promise<void> {
        try {
            ...
            logger.error("[UserManager] refreshToken failed", { error: err });
        }
    }
}
