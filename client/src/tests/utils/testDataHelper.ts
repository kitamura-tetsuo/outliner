...
export function logDebugInfo(): void {
    ...
    const userManager = (window as unknown as {
        __USER_MANAGER__?: { getCurrentUser?: () => unknown; auth?: { currentUser?: unknown; }; };
    }).__USER_MANAGER__;
    console.log("Current user:", userManager?.getCurrentUser?.());
    console.log("Auth state:", userManager?.auth?.currentUser);
    ...
    if (isTestEnv) {
        (window as unknown as {
            __TEST_DATA_HELPER__?: {
                ...
                performTestLogin: typeof performTestLogin;
                logDebugInfo: typeof logDebugInfo;
            };
        }).__TEST_DATA_HELPER__ = {
            createTestUserData,
            clearTestData,
