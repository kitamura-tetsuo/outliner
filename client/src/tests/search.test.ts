...
function setupTree() {
    ...
    const child1 = (page.items as unknown).addNode("user");
    child1.updateText("hello world");
    const child2 = (page.items as unknown).addNode("user");
...
describe("search utilities", () => {
    it("finds matches case insensitive", () => {
        ...
        const results = searchItems(page as unknown, "hello", {});
    ...
    it("respects case sensitivity", () => {
        const { page } = setupTree();
        const results = searchItems(page as unknown, "hello", { caseSensitive: true });
        const total = results.reduce((c, r) => c + r.matches.length, 0);
        expect(total).toBe(1);
    });
    ...
    it("replace functions modify text", () => {
        ...
        const replaced = replaceFirst(page as unknown, "hello", "hi");
        expect(replaced).toBe(true);
        expect(child1.text.includes("hi")).toBe(true);
        replaceAll(page as unknown, "world", "earth");
