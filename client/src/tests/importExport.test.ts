...
describe("Import/Export Service", () => {
    describe("Markdown Import", () => {
        it("should import simple markdown with child items", () => {
            ...
            const page = project.items[0];
            expect(page.text).toBe("ImportedPage");

            // ページに1つの子アイテムがあるはず
            expect(page.items.length).toBe(1);
            expect(page.items[0].text).toBe("Child");
        });
        ...
        it("should import nested markdown with multiple levels", () => {
            ...
            // ページに1つの子アイテムがあるはず
            expect(page.items.length).toBe(1);
            const child = page.items[0];
            expect(child.text).toBe("Child");

            // 子アイテムに1つの孫アイテムがあるはず
            expect(child.items.length).toBe(1);
            expect(child.items[0].text).toBe("Grand");
        });
        ...
        it("should import multiple root items correctly", () => {
            ...
            expect(page.items[0].text).toBe("Child1");
            expect(page.items[1].text).toBe("SecondItem");

            // SecondItemに1つの子アイテムがあるはず
            expect(page.items[1].items.length).toBe(1);
            expect(page.items[1].items[0].text).toBe("Child2");
        });
    });
