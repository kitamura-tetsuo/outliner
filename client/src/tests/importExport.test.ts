...
describe("Import/Export Service", () => {
    describe("Markdown Import", () => {
        it("should import simple markdown with child items", () => {
            ...
            expect((project.items as unknown).length).toBe(1);

            const page = (project.items as unknown)[0];
            expect(page.text).toBe("ImportedPage");

            // ページに1つの子アイテムがあるはず
            expect((page.items as unknown).length).toBe(1);
            expect((page.items as unknown)[0].text).toBe("Child");
        });
        ...
        it("should import nested markdown with multiple levels", () => {
            ...
            // ページに1つの子アイテムがあるはず
            expect((page.items as unknown).length).toBe(1);
            const child = (page.items as unknown)[0];
            expect(child.text).toBe("Child");

            // 子アイテムに1つの孫アイテムがあるはず
            expect((child.items as unknown).length).toBe(1);
            expect((child.items as unknown)[0].text).toBe("Grand");
        });
        ...
        it("should import multiple root items correctly", () => {
            ...
            expect((page.items as unknown)[0].text).toBe("Child1");
            expect((page.items as unknown)[1].text).toBe("SecondItem");

            // SecondItemに1つの子アイテムがあるはず
            expect((page.items as unknown)[1].items.length).toBe(1);
            expect((page.items as unknown)[1].items[0].text).toBe("Child2");
        });
    });
    ...
    describe("Export", () => {
        it("should export project to markdown", () => {
            ...
            const child = (page.items as unknown).addNode("test");
