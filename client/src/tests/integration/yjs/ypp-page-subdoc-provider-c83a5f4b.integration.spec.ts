...
describe("page subdoc provider", () => {
    it("connects each page to its own room with isolated awareness", async () => {
        ...
        const c1 = await new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if (!resolved) {
                    console.log("Timeout waiting for page connection for p1");
                    reject(new Error("Timeout waiting for page connection for p1"));
                }
            }, 10000);
        });
        ...
        const c2 = await new Promise((resolve, reject) => {
            ...
            setTimeout(() => {
                if (!resolved) {
                    console.log("Timeout waiting for page connection for p2");
                    reject(new Error("Timeout waiting for page connection for p2"));
                }
            }, 10000);
        });
