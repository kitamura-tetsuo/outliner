    ...
    it("propagates cursor between clients", async () => {
        ...
        const p1c1 = await new Promise<PageConnection | null>((resolve) => {
            ...
            const check = () => {
                ...
                if (conn) {
                    resolved = true;
                    resolve(conn as PageConnection);
                } else if (!resolved) {
                    setTimeout(check, 50);
                }
            };
        ...
        const states1 = p1c1.awareness.getStates() as Map<number, any>;
        const states2 = p1c2.awareness.getStates() as Map<number, any>;
        ...
        const states = p1c2.awareness.getStates() as Map<number, any>;
