...
describe("CNT: ContainerSelector option count reflects accessibleContainerIds", () => {
    beforeEach(() => {
        ...
        (globalThis as unknown).window ||= globalThis as unknown;
        (globalThis as unknown).window.__USER_MANAGER__ = {
    ...
    it("option 数が accessibleContainerIds の増減に合わせて変化する", async () => {
        ...
        (firestoreStore.userContainer!.accessibleContainerIds as unknown).push("c-2");
        ...
        (firestoreStore.userContainer!.accessibleContainerIds as unknown).pop();
        await waitFor(() => {
            expect(within(select).getAllByRole("option").length).toBe(1);
        });
    });
