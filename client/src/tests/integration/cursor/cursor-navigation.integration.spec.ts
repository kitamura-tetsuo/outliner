...
describe("Cursor Integration", () => {
    ...
    beforeEach(() => {
        ...
        (generalStore as unknown).currentPage = mockParentItem;
    ...
    describe("Text Operations", () => {
        it("should handle text insertion with selection", () => {
            ...
            // Mock a selection
            (editorOverlayStore as unknown).selections = {
                "selection-1": {
                    userId: "user-1",
        ...
        it("should handle line breaks correctly", () => {
            ...
            if (mockItem.parent) {
                (mockItem.parent as unknown).indexOf = (item: Item) => {
                    if (item === mockItem) return 0;
                    return -1;
                };
                (mockItem.parent as unknown).addNode = vi.fn().mockReturnValue({
                    id: "new-item-1",
                    text: "World",
                    updateText: vi.fn(),
                    delete: vi.fn(),
            ...
            expect(
                (mockItem.parent as unknown).addNode || mockParentItem.items.addNode,
            ).toHaveBeenCalled();
        });
    });
