...
describe("Cursor moveToDocumentStart and End", () => {
    ...
    test("extendSelectionToDocumentStart extends selection to the first item", () => {
        ...
        // Mock updateSelectionAfterMove to verify it was called correctly
        type CursorMocks = {
            updateSelectionAfterMove: import("vitest").Mock;
            getSelection: import("vitest").Mock;
            getSelectionForCurrentItem: import("vitest").Mock;
        };
        (cursor as unknown as CursorMocks).updateSelectionAfterMove = vi.fn();

        // Mock getSelection
        (cursor as unknown as CursorMocks).getSelection = vi.fn().mockReturnValue(null);
        (cursor as unknown as CursorMocks).getSelectionForCurrentItem = vi.fn().mockReturnValue(null);
    ...
    test("extendSelectionToDocumentEnd extends selection to the last item", () => {
        const firstItem = page.items.at(0)!;
        cursor.itemId = firstItem.id;
        cursor.offset = 2;
        ...
        cursor.extendSelectionToDocumentEnd();
