...
interface Props {
    ...
    item?: {
        ...
        comments?: Comments;
        tree?: {
            getNodeValueFromKey: (key: string) => Y.Map<unknown>;
        };
        key?: string;
    };
}
...
function e2eLog(entry: E2ELogEntry) {
    try {
        const w = window as Window & { E2E_LOGS?: Array<Record<string, unknown>> };
...
onMount(() => {
    ...
    try {
        ...
        let yarr: Y.Array<Y.Map<unknown>> | null = (comments as Comments & { yArray?: Y.Array<Y.Map<unknown>> })?.yArray ?? null;
...
function add() {
    ...
    const time = Date.now();
    let id: string;
    if (commentsObj && typeof (commentsObj as Comments).addComment === 'function') {
        const res = (commentsObj as Comments).addComment(user, newText);
    ...
    try {
        ...
        let countNow = 0;
        if (commentsObj && typeof (commentsObj as Comments).length === 'number') {
            countNow = (commentsObj as Comments).length;
        } else {
            ...
            try {
                if (props.item && typeof props.item.comments !== 'undefined') {
                    ...
                    if (typeof (itemComments as Comments).length === 'number') {
                        countNow = (itemComments as Comments).length;
                    }
                }
            } catch {}
        }
