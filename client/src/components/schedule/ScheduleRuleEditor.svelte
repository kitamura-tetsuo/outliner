...
if (initialRule.rrule) {
    try {
        const parsed = rrulestr(initialRule.rrule);
...
// Shared state
let sql = $state(initialRule.sql || `INSERT INTO "${tableId}" (id, occurrence_time) VALUES (gen_random_uuid(), current_setting('job.occurrence')::timestamptz);`);
let dtstart = $state(initialRule.dtstart || "");
let timezone = $state(initialRule.timezone || Intl.DateTimeFormat().resolvedOptions().timeZone);
let enabled = $state(initialRule.enabled !== false);
let rawRruleStr = $state(initialRule.rrule || "FREQ=DAILY;INTERVAL=1");
...
let byweekday = $state<number[]>((initialParsed?.options.byweekday || []).map((w: unknown) => {
    ...
    return typeof w === 'object' && w !== null && 'weekday' in w ? (w as { weekday: number }).weekday : w as number;
}));
