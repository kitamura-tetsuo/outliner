<script lang="ts">
    ...
    import { createFluidClient, getFluidClient } from "../lib/fluidService";
    ...
    const loadContainers = async () => {
        ...
        try {
            // userContainerからコンテナリストを作成
            let userContainers: Array<{
                id: string;
                name: string;
                isDefault?: boolean;
            }> = [];

            if (firestoreStore.userContainer) {
                // デフォルトコンテナがあれば優先的に追加
                if (firestoreStore.userContainer.defaultContainerId) {
                    const instance = await getFluidClient(
                        firestoreStore.userContainer.defaultContainerId,
                    );
                    userContainers.push({
                        id: firestoreStore.userContainer.defaultContainerId,
                        name: instance[4]?.title!,
                        isDefault: true,
                    });
                }
                ...
                if (
                    firestoreStore.userContainer.accessibleContainerIds &&
                    firestoreStore.userContainer.accessibleContainerIds.length >
                        0
                ) {
                    // デフォルトコンテナと重複しないようにフィルタリング
                    const additionalContainers = await Promise.all(
                        firestoreStore.userContainer.accessibleContainerIds
                            .filter(
                                (id: string) =>
                                    id !==
                            ...
                            .map(async (id: string) => ({
                                id,
                                name: (await getFluidClient(id))[4]?.title!, // IDの一部を表示
                                isDefault: false,
                            })),
                    );

                    userContainers.push(...additionalContainers);
                }
            }
