...
onMount(() => {
    ...
    // フォールバック: Palette visible: Forward character input/move/confirm from global keydown directly
    try {
        const paletteTypeForwarder = (ev: KeyboardEvent) => {
            ...
            if (!ev.ctrlKey && !ev.metaKey && !ev.altKey && k.length === 1 && k !== "/") {
                // Immediate UI update with lightweight input (model unchanged)
                cps.inputLight(k);
                ev.preventDefault();
                return;
            }
            ...
            if (k === "Enter") {
                try {
                    // If single filter result and Alias, insert directly then close (ensure picker shows)
                    ...
                    // Strict detection from textarea value directly
    ...
    // Global: Backup calling KeyEventHandler regardless of focus position
    try {
        const globalKeyForwarder = (ev: KeyboardEvent) => {
            ...
            const isBoxSelectionKey = ev.altKey && ev.shiftKey &&
                (ev.key === "ArrowUp" || ev.key === "ArrowDown" || ev.key === "ArrowLeft" || ev.key === "ArrowRight");
            if (ev.isComposing || (!isBoxSelectionKey && (ev.ctrlKey || ev.metaKey || ev.altKey))) return;
            // Always delegate to KeyEventHandler (processed internally only if needed)
...
function updateCompositionWidth(text: string) {
    if (!textareaRef || !measureCtx) {
        // Fallback: Set fixed width if measureCtx unavailable
        if (textareaRef) {
 ...
 * Since `KeyEventHandler.handlePaste` returns Promise, `await` it
 * catch permission denial or read failure, and `clipboard-permission-denied`
 * or `clipboard-read-error` is dispatched and not pasted to user.
 */
