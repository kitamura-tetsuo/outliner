...
// Type declarations for global window properties used for testing and fallbacks
declare global {
    interface Window {
    ...
    try {
        const recordKeys = (ev: KeyboardEvent) => {
            ...
            if (typeof k === "string" && k.length === 1) {
                window.__KEYSTREAM__ = (window.__KEYSTREAM__ || "") + k;
                if (window.__KEYSTREAM__!.length > 256) {
                    window.__KEYSTREAM__ = window.__KEYSTREAM__!.slice(-256);
                }
            }
        };
