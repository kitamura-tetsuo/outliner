<script lang="ts">
    ...
    function navigateToPage(page?: Item) {
        ...
        if (targetPage) {
            ...
            goto(
                `/${encodeURIComponent(projTitle)}/${encodeURIComponent(title)}`,
            );
        } else if (query) {
    ...
    onMount(() => {
        try {
            ...
            // Additional visibility check (Metric close to Playwright)
            ...
            const cx = rect
                ? Math.max(
                      0,
                      Math.min(vw - 1, rect.left + Math.min(rect.width - 1, 5)),
                  )
                : 0;
            const cy = rect
                ? Math.max(
                ...
                : 0;
            const eCenter = (
                rect ? document.elementFromPoint?.(cx, cy) : null
            ) as Element | null;
            const eCenterInfo = eCenter
                ? {
                      tag: eCenter.tagName,
                      id: (eCenter as HTMLElement).id,
            ...
            // Measure by tracing parent chain computed styles upwards
            type ChainInfo = {
                tag: string;
                id: string;
                cls: string;
