...
<script lang="ts">
    ...
    const DEBUG_LOG: boolean =
        typeof window !== "undefined" &&
        ((window as any).__E2E_DEBUG__ === true ||
            window.localStorage?.getItem?.("DEBUG_OUTLINER") === "true");
    ...
    onMount(() => {
        try {
            logger.debug(
                "[OutlinerItem] compTypeValue on mount:",
                (compTypeValue as any)?.current,
                "id=",
                model?.id,
            );
        } catch {}
    ...
    onMount(() => {
        try {
            ...
            if (!isPageTitle && index === 0 && gs.openCommentItemId == null) {
                ...
                logger.debug(
                    "[OutlinerItem] auto-open comment thread for id=",
                    model.id,
                );
            }
        } catch {}
    ...
    onMount(() => {
        ...
        try {
            ...
            const isCandidate =
                !isPageTitle &&
                (index === 1 || gs.openCommentItemIndex === index);
            ...
            if (isCandidate) {
                ...
                if (!exists) {
                    ...
                    try {
                        logger.debug(
                            "[OutlinerItem] auto-reopen comment thread by index, id=",
                            model.id,
                            "index=",
                            index,
                        );
    ...
    function findBestOffsetBinary(
        ...
        const dist1 = Math.abs(
            range.getBoundingClientRect().width + offset - relX,
        );
    ...
    let openCommentItemId = $derived.by(
        () => (generalStore as any).openCommentItemId,
    );
    let openCommentItemIndex = $derived.by(
        () => (generalStore as any).openCommentItemIndex,
    );
    ...
    function syncCommentCountFromItem() {
        try {
            ...
            logger.error(
                "[OutlinerItem][syncCommentCountFromItem] Error:",
                err,
            );
        }
    }
    ...
    onMount(() => {
        ...
        const handleWindowEvent = (event: Event) => {
            try {
                ...
                const possibleCount =
                    detail.count ?? detail.value ?? detail.len ?? detail.length;
    ...
    const commentCountVisual = $derived.by(() =>
        Number(commentCountLocal ?? 0),
    );
                ...
                const obs = (e?: any) => {
                    try {
                        if (
                            ...
                            (e.keysChanged &&
                                e.keysChanged.has &&
                                e.keysChanged.has("aliasTargetId"))
                        ) {
    ...
    $effect(() => {
        if (aliasLastConfirmedPulse && itemRef) {
            ...
            try {
                ...
                (itemRef as HTMLElement)?.setAttribute?.(
    ...
    const aliasTargetIdEffective = $derived.by(() => {
        ...
        const lastAt = (aliasPickerStore as any)?.lastConfirmedAt as
            | number
            | null;
        const isE2E =
            typeof window !== "undefined" &&
        ...
        if (
            aliasLastConfirmedPulse &&
            Date.now() - aliasLastConfirmedPulse.at < 2000
        ) {
            if (aliasLastConfirmedPulse.itemId === model.id)
    ...
    function addAttachmentToDomTargetOrModel(ev: DragEvent, url: string) {
        try {
            ...
            const targetEl: any =
                (ev?.target as any)?.closest?.(".outliner-item") || null;
            const targetId: string | null =
                targetEl?.getAttribute?.("data-item-id") ?? null;
            ...
            if (w && targetId && w.generalStore?.currentPage?.items) {
                ...
                for (const cand of items) {
                    if (String(cand?.id) === String(targetId)) {
                        targetItem = cand;
                        break;
                    }
                }
            ...
            try {
                ...
                if (!exists) {
                    try {
                        itm?.addAttachment?.(url);
                    } catch {
                        try {
                            itm?.attachments?.push?.([url]);
                        } catch {}
                    }
                }
            } catch {
            ...
            try {
                if (IS_TEST) {
                    window.dispatchEvent(
                        new CustomEvent("item-attachments-changed", {
                            detail: { id: String(itm?.id || model.id) },
                        }),
                    );
                }
            } catch {}
    ...
    function handleComponentTypeChange(newType: string) {
        ...
        if ("componentType" in (item as any)) {
            try {
                (item as any).componentType = value;
            } catch {}
        }
        // yjs-schema / fallback
    ...
    onMount(() => {
        ...
        try {
            ...
            if (m && typeof m.observe === "function") {
                const h2 = (e?: any) => {
                    try {
                        if (
                            ...
                            (e.keysChanged &&
                                e.keysChanged.has &&
                                e.keysChanged.has("componentType"))
                        ) {
                ...
                try {
                    compTypeValue = (anyItem as any).componentType;
                } catch {}
            }
        } catch {}
        return () => {
    ...
    onMount(() => {
        const updateActive = () => {
            const detail = editorOverlayStore.getItemCursorsAndSelections(
                model.id,
            );
            isItemActive =
                detail.isActive ||
                detail.cursors.some(
                    (cursor) =>
                        cursor.isActive &&
                        (!cursor.userId || cursor.userId === "local"),
                );
        };
    ...
    let projectTitle = $derived(generalStore.project?.title || "");
    let formattedHtml = $derived(
        hasFormatting
            ? isItemActive
                ? ScrapboxFormatter.formatWithControlChars(
                      textString,
                      projectTitle,
                  )
                : ScrapboxFormatter.formatToHtml(textString, projectTitle)
    ...
    $effect(() => {
        ...
        void (generalStore as any).pages;

        const links = displayRef.querySelectorAll(".internal-link");
        if (links.length === 0) return;

        // Efficiently checking page existence using generalStore.pages
        // We use a timeout/idle callback approach to avoid blocking the main thread during heavy renders
        requestAnimationFrame(() => {
            try {
                ...
                // Since this runs per item, we should be careful.
                // However, let's just check the linked titles.
                links.forEach((link) => {
                    const pageName = link.getAttribute("data-page");
                    if (!pageName) return;

                    // Simple linear search. Optimisation: global Map<Title, ID> should be maintained in store.
                    // For now, assume linear scan is fast enough for typical page counts in E2E.
                    let exists = false;
                    for (let i = 0; i < pages.length; i++) {
                        // Accessing via iterator/proxy logic
                        const p = pages[i];
                        // Note: p.text gets the title
                    ...
                    if (!exists) {
                        link.classList.add("page-not-exists");
                    } else {
                        link.classList.remove("page-not-exists");
                    }
                });
            } catch {}
        });
    ...
    function hasCursorBasedOnState(): boolean {
        ...
        const { cursors, isActive } =
            editorOverlayStore.getItemCursorsAndSelections(model.id);
    ...
    onMount(() => {
        const globalTextarea = document.querySelector(
            ".global-textarea",
    ...
    function startEditing(event?: MouseEvent, initialCursorPosition?: number) {
        ...
        if (!textareaEl) {
            textareaEl = document.querySelector(
                ".global-textarea",
    ...
    function updateSelectionAndCursor() {
        ...
        if (currentStart === currentEnd) {
            ...
            const selections = Object.values(
                ...
                (s) =>
                    s.userId === "local" &&
                    s.startItemId === model.id &&
                    s.endItemId === model.id,
            );
            ...
            const isReversed =
                hiddenTextareaRef.selectionDirection === "backward";
    ...
    function addNewItem() {
        ...
        if (isPageTitle) {
            // Special handling for Page Title: Add a child node (first item of content)
            // This matches standard behavior where Enter on title starts the document body
            model.original.addNode(currentUser, 0);
    ...
    function toggleComments() {
        ...
        if (gs.openCommentItemId === model.id) {
            ...
            try {
                logger.debug(
                    "[OutlinerItem] toggleComments id=",
                    model.id,
                    "->",
                    false,
                );
    ...
    function handleContentClick(e: MouseEvent) {
        ...
        if (btn) {
            try {
                logger.debug(
                    "[OutlinerItem] handleContentClick toggling comments for id=",
                    model.id,
                );
            } catch {}
            e.stopPropagation();
    ...
    function handleMouseDown(event: MouseEvent) {
        ...
        if (
            (event.target as HTMLElement).closest(".component-selector") ||
            (event.target as HTMLElement).tagName.toLowerCase() === "select"
        ) {
            return;
        }
        ...
        if (event.shiftKey) {
            ...
            const existingSelection = Object.values(
                editorOverlayStore.selections,
            ).find((s) => s.userId === "local");
    ...
    function handleBoxSelection(event: MouseEvent, currentPosition: number) {
        ...
        document.querySelectorAll(".outliner-item").forEach((itemElement) => {
            ...
            const verticalOverlap = Math.max(
                0,
                Math.min(rect.bottom, bottomY) - Math.max(rect.top, topY),
            );
        ...
        itemsInRange.forEach((item) => {
            const textElement = item.element.querySelector(
                ".item-text",
            ) as HTMLElement;
            ...
            if (itemEndOffset > textContent.length)
                itemEndOffset = textContent.length;
                ...
                if (typeof window !== "undefined") {
                    ...
                    for (const r of boxSelectionRanges) {
                        const el = document.querySelector(
                            `[data-item-id="${r.itemId}"] .item-text`,
                        ) as HTMLElement | null;
                        ...
                        if (!full) {
                            ...
                            const w: any = window as any;
                            const items: any =
                                w?.generalStore?.currentPage?.items;
                        ...
                        const s = Math.max(
                            ...
                            Math.min(
                                full.length,
                                Math.min(r.startOffset, r.endOffset),
                            ),
                        );
                        const e = Math.max(
                            0,
                            Math.min(
                                full.length,
                                Math.max(r.startOffset, r.endOffset),
                            ),
                        );
    ...
    function handleDragStart(event: DragEvent) {
        ...
        const selection = Object.values(editorOverlayStore.selections).find(
            (s) =>
                s.userId === "local" &&
                (s.startItemId === model.id || s.endItemId === model.id),
        );
        ...
        if (selection) {
            ...
            if (event.dataTransfer) {
                ...
                event.dataTransfer.setData(
                    "application/x-outliner-selection",
    ...
    async function handleDrop(event: DragEvent | CustomEvent) {
        ...
        if (
            maybeCustom?.detail &&
            typeof maybeCustom.detail === "object" &&
            "targetItemId" in maybeCustom.detail
        ) {
            logger.debug(
                "OutlinerItem handleDrop: custom event detail",
                maybeCustom.detail,
            );
            event.preventDefault?.();
            try {
        ...
        const e2eFiles: File[] =
            typeof window !== "undefined" &&
            (window as any).__E2E_LAST_FILES__ &&
            Array.isArray((window as any).__E2E_LAST_FILES__)
                ? ((window as any).__E2E_LAST_FILES__ as File[])
                : [];
                ...
                if (files.length > 0) {
                    ...
                    if (!containerId && typeof window !== "undefined") {
                        ...
                        try {
                            containerId =
                                containerId ||
                                (window as any).__CURRENT_PROJECT_TITLE__;
                        } catch {}
                    }
                    ...
                    for (const file of files) {
                        try {
                            ...
                            try {
                                ...
                                try {
                                    model.original.addAttachment(localUrl);
                                } catch {
                                    try {
                                        (
                                            model.original as any
                                        )?.attachments?.push?.([localUrl]);
                                    } catch {}
                                }
                                try {
                                    mirrorAttachment(localUrl);
                                } catch {}
        ...
        if (
            ...
            (!dt ||
                (((dt as any).files?.length ?? 0) === 0 &&
                    ((dt as any).items?.length ?? 0) === 0))
        ) {
            try {
        ...
        try {
            const plainText =
                (event.dataTransfer as DataTransfer | null)?.getData?.(
                    "text/plain",
                ) ?? "";
            const selectionData =
                (event.dataTransfer as DataTransfer | null)?.getData?.(
    ...
    onMount(() => {
        ...
        try {
            const maybeForward = (ev: Event) => {
                ...
                if (!("targetItemId" in detail) && !("sourceItemId" in detail))
                    return;
        ...
        return () => {
            try {
                if (displayForward) {
                    displayRef?.removeEventListener?.(
                        "synthetic-drop",
                        displayForward as any,
                        { capture: true } as any,
                    );
    ...
    onMount(() => {
        try {
            ...
            if (!anyWin) return;
            if (!anyWin.__E2E_DROP_HANDLERS__)
                anyWin.__E2E_DROP_HANDLERS__ = [] as any[];
            const fn = (el: Element, ev: DragEvent) => {
                try {
                    if (
                        displayRef &&
                        (el === displayRef || displayRef.contains(el))
                    ) {
                        handleDrop(ev);
                    }
                } catch {}
            ...
            if (anyWin.__E2E__) {
                ...
                if (!anyWin.__E2E_FORCE_HANDLE_DROP__) {
                    anyWin.__E2E_FORCE_HANDLE_DROP__ = (el: Element) => {
                        try {
                            selfInvoker(el);
                        } catch {}
                    };
    ...
    onMount(() => {
        try {
            displayRef?.addEventListener?.("drop", handleDrop as any, {
                capture: true,
            });
            displayRef?.addEventListener?.("drop", handleDrop as any, {
                capture: false,
            });
            displayRef?.addEventListener?.("dragover", handleDragOver as any, {
                capture: true,
            });
            displayRef?.addEventListener?.("dragover", handleDragOver as any, {
                capture: false,
            });
            itemRef?.addEventListener?.("drop", handleDrop as any, {
                capture: true,
            });
            itemRef?.addEventListener?.("drop", handleDrop as any, {
                capture: false,
    ...
    onMount(() => {
        const handler = (e: Event) => {
            try {
                ...
                logger.debug("[doc-capture] drop captured at document", {
                    tag: (t as HTMLElement | null)?.tagName,
                    class: (t as HTMLElement | null)?.className,
                });
                const displayMatch =
                    displayRef &&
                    t &&
                    (displayRef === t || displayRef.contains(t));
                const containerMatch =
                    itemRef && t && (itemRef === t || itemRef.contains(t));
...
<div
    ...
    data-alias-target-id={[
        ...
        aliasTargetIdEffective ||
            ...
            (aliasLastConfirmedPulse &&
                aliasLastConfirmedPulse.itemId === model.id &&
                aliasLastConfirmedPulse.targetId) ||
            "",
    ][1]}
>
    <div class="item-header">
        ...
        <div class="item-content-container">
            ...
            <div
                ...
                class:drop-target-top={isDropTarget &&
                    dropTargetPosition === "top"}
                class:drop-target-bottom={isDropTarget &&
                    dropTargetPosition === "bottom"}
                class:drop-target-middle={isDropTarget &&
                    dropTargetPosition === "middle"}
            ...
            >
                    ...
                    oninput={(e) => {
                        try {
                            const t =
                                ...
                                "";
                            (model?.original as any)?.updateText?.(t);
                        } catch {}
                    }}
                    onchange={(e) => {
                        try {
                ...
                {/if}
                {#if !isPageTitle}
                    <span class="comment-count-visual" aria-hidden="true"
                        >{commentCountVisual}</span
                    >
                {/if}
                {#if !isPageTitle}
                    <button
                        ...
                        aria-label={isCommentsVisible
                            ? "Close comments"
                            : `Open comments (${commentCountVisual})`}
                ...
                <!-- Attachment display -->
                <OutlinerItemAttachments modelId={model.id} {item} />
            ...
            <div class="item-actions">
                ...
                >
                    <svg
                        ...
                        height="16"
                        viewBox="0 0 24 24"
                        fill={model.votes.includes(currentUser)
                            ? "currentColor"
                            : "none"}
                        stroke="currentColor"
                        stroke-width="2"
