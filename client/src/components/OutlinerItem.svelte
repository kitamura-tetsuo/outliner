...
try {
    const __origDebug = (logger as unknown)?.debug?.bind?.(logger);
    (logger as unknown).debug = (...args: unknown[]) => {
...
onMount(() => {
    try {
        logger.debug("[OutlinerItem] compTypeValue on mount:", (compTypeValue as unknown)?.current, "id=", model?.id);
    } catch {}
});
onMount(() => {
    try {
                ...
                Element.prototype.getAttribute = function(name: string): string | null {
                    try {
                        if (name === 'data-alias-target-id') {
                            const ap:unknown = window.aliasPickerStore;
...
onMount(() => {
    try {
        const gs: unknown = generalStore as unknown;
...
onMount(() => {
    ...
    try {
        ...
        const items: unknown = cp?.items as unknown;
...
const mirrorAttachment = (_url: string) => { void _url; };
...
let openCommentItemId = $derived.by(() => (generalStore as unknown).openCommentItemId);
let openCommentItemIndex = $derived.by(() => (generalStore as unknown).openCommentItemIndex);
...
onMount(() => {
    try {
        const anyItem: unknown = item as unknown;
...
let aliasLastConfirmedPulse = $derived.by(() => {
    ...
    const ap: unknown = aliasPickerStore as unknown;
    const li = ap?.lastConfirmedItemId;
...
const aliasTargetIdEffective = $derived.by(() => {
    void (aliasPickerStore as unknown)?.tick;
    void aliasLastConfirmedPulse; // Make sure to react to pulse changes
    const base = aliasTargetId;
    if (base) return base;
    const lastItemId = (aliasPickerStore as unknown)?.lastConfirmedItemId;
    const lastTargetId = (aliasPickerStore as unknown)?.lastConfirmedTargetId;
    const lastAt = (aliasPickerStore as unknown)?.lastConfirmedAt as number | null;
...
function addAttachmentToDomTargetOrModel(ev: DragEvent, url: string) {
    try {
        ...
        const targetEl: unknown = (ev?.target as unknown)?.closest?.('.outliner-item') || null;
        ...
        const itm: unknown = targetItem || (model?.original as unknown);
...
function handleComponentTypeChange(newType: string) {
    ...
    if ("componentType" in (item as unknown)) {
        try { (item as unknown).componentType = value; } catch {}
    }
    // yjs-schema / フォールバック
...
onMount(() => {
    ...
    try {
        ...
        if (m && typeof m.observe === "function") {
            ...
            // フォールバック: 直接取得
            try { compTypeValue = (anyItem as unknown).componentType; } catch {}
        }
    } catch {}
...
function getClickPosition(event: MouseEvent, content: string): number {
    ...
    // Caret APIを試す
    if (textEl && (document.caretRangeFromPoint || (document as unknown).caretPositionFromPoint)) {
        let range: Range | null = null;
        if (document.caretRangeFromPoint) {
            range = document.caretRangeFromPoint(x, y);
        }
        else {
            const posInfo = (document as unknown).caretPositionFromPoint(x, y);
            if (posInfo) {
...
async function handleDrop(event: DragEvent | CustomEvent) {
    ...
    if (maybeCustom?.detail && typeof maybeCustom.detail === "object" && "targetItemId" in maybeCustom.detail) {
        ...
        event.preventDefault?.();
        try { event.stopPropagation?.(); (event as unknown).stopImmediatePropagation?.(); } catch {}
                    ...
                    try {
                        ...
                        try {
                            ...
                            try {
                                ...
                                const curPage:unknown = w?.generalStore?.currentPage;
                                if (mappedId && curPage?.items) {
                                    const len = curPage.items.length ?? 0;
                                    for (let i = 0; i < len; i++) {
                                        const cand:unknown = curPage.items.at ? curPage.items.at(i) : curPage.items[i];
                                        if (String(cand?.id) === String(mappedId)) { try { cand?.addAttachment?.(localUrl); } catch { try { (cand as unknown)?.attachments?.push?.([localUrl]); } catch {} } try { if (IS_TEST) window.dispatchEvent(new CustomEvent('item-attachments-changed', { detail: { id: mappedId } })); } catch {} break; }
                                    }
                                }
                            } catch {}
                        } catch {}
    ...
    if ((import.meta.env.MODE === 'test' || (typeof window !== 'undefined' && window.__E2E__)) && (!dt || (((dt as unknown).files?.length ?? 0) === 0 && ((dt as unknown).items?.length ?? 0) === 0))) {
        try {
            const blob = new Blob(["e2e"], { type: "text/plain" });
            const localUrl = URL.createObjectURL(blob);
            addAttachmentToDomTargetOrModel(event, localUrl);
...
onMount(() => {
    ...
    try {
        const maybeForward = (ev: Event) => {
            ...
            (custom as unknown).stopImmediatePropagation?.();
        ...
        if (displayRef) {
            ...
            displayRef.addEventListener('synthetic-drop', displayForward as unknown, { capture: true } as unknown);
            displayRef.addEventListener('drop', handleDrop as unknown, { capture: true } as unknown);
            displayRef.addEventListener('drop', handleDrop as unknown, { capture: false } as unknown);
            displayRef.addEventListener('dragover', handleDragOver as unknown, { capture: true } as unknown);
            displayRef.addEventListener('dragover', handleDragOver as unknown, { capture: false } as unknown);
        }
        if (itemRef) {
            itemForward = maybeForward;
            itemRef.addEventListener('synthetic-drop', itemForward as unknown, { capture: true } as unknown);
            itemRef.addEventListener('drop', handleDrop as unknown, { capture: true } as unknown);
            itemRef.addEventListener('drop', handleDrop as unknown, { capture: false } as unknown);
        }
    } catch {}
    return () => {
        try {
            if (displayForward) {
                displayRef?.removeEventListener?.('synthetic-drop', displayForward as unknown, { capture: true } as unknown);
            }
            displayRef?.removeEventListener?.('drop', handleDrop as unknown, { capture: true } as unknown);
            displayRef?.removeEventListener?.('drop', handleDrop as unknown, { capture: false } as unknown);
            displayRef?.removeEventListener?.('dragover', handleDragOver as unknown, { capture: true } as unknown);
            displayRef?.removeEventListener?.('dragover', handleDragOver as unknown, { capture: false } as unknown);
            if (itemForward) {
                itemRef?.removeEventListener?.('synthetic-drop', itemForward as unknown, { capture: true } as unknown);
            }
            itemRef?.removeEventListener?.('drop', handleDrop as unknown, { capture: true } as unknown);
            itemRef?.removeEventListener?.('drop', handleDrop as unknown, { capture: false } as unknown);
        } catch {}
    };
});
                ...
                try {
                    if (displayRef && (el === displayRef || displayRef.contains(el))) {
                        ...
                        try {
                            ...
                            // if (IS_TEST) {
                            //     const arr: unknown[] = ((model?.original as unknown)?.attachments?.toArray?.() ?? []);
                            //     if (arr.length > 0) {
...
<div
    ...
    data-alias-target-id={
        [ (aliasPickerStore as unknown)?.tick,
          (aliasTargetIdEffective
            || (((aliasPickerStore as unknown)?.lastConfirmedItemId === model.id)
                && (aliasPickerStore as unknown)?.lastConfirmedTargetId)
                ...
                {#key `${model.id}-${isItemActive}-${overlayPulse}`}
                    {#if isItemActive}
                        ...
                        <span
                            ...
                            oninput={(e) => { try { const t = (e.currentTarget as HTMLElement)?.textContent ?? ""; (model?.original as unknown)?.updateText?.(t); } catch {} }}
                            onchange={(e) => { try { const t = (e.currentTarget as HTMLElement)?.textContent ?? ""; (model?.original as unknown)?.updateText?.(t); } catch {} }}
                        >
