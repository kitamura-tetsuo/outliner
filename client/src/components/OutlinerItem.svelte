...
const _localLogger = getLogger("OutlinerItem.svelte");
...
try {
    if (hasDebug(logger)) {
        ...
        logger.debug = (...args: unknown[]) => {
            if (DEBUG_LOG && __origDebug) { try { __origDebug(...args); } catch (err) { _localLogger.warn("Silenced error", { err }); } }
        };
    }
...
function attachCommentObserver(): (() => void) | null {
    try {
        ...
        const attachToArray = () => {
            try { detachArray?.(); } catch (err) { _localLogger.warn("Silenced error", { err }); }
            detachArray = null;
            const arr: unknown[] = ensureCommentsArray();
            if (hasObserve(arr)) {
        ...
        return () => {
            try { detachArray?.(); } catch (err) { _localLogger.warn("Silenced error", { err }); }
            try { detachMap?.(); } catch (err) { _localLogger.warn("Silenced error", { err }); }
        };
    } catch (err) { _localLogger.warn("Silenced error", { err }); }
    return null;
    }
...
onMount(() => {
    ...
    return () => {
        for (const fn of cleanup) {
            try { fn(); } catch (err) { _localLogger.warn("Silenced error", { err }); }
        }
    };
});
    ...
    try {
        if (hasTreeKey(item)) {
            ...
            if (ymap && typeof ymap.observe === 'function') {
                ...
                // Initial reflection
                obs();
                return () => { try { ymap?.unobserve?.(obs); } catch (err) { _localLogger.warn("Silenced error", { err }); } };
...
function handleComponentTypeChange(newType: string) {
    ...
    if (item && typeof item === "object" && "componentType" in item) {
        try { if (hasComponentType(item)) { item.componentType = value; } } catch (err) { _localLogger.warn("Silenced error", { err }); }
    }
    // yjs-schema / fallback
...
$effect(() => {
    ...
    try {
        if (hasTreeKey(item)) {
            ...
            t.observe(h1); unsubs.push(() => { try { t.unobserve?.(h1); } catch (err) { _localLogger.warn("Silenced error", { err }); } });
            // Initial reflection
            h1();
        }
            if (m && typeof m.observe === "function") {
                ...
                m.observe(h2); unsubs.push(() => { try { m.unobserve?.(h2); } catch (err) { _localLogger.warn("Silenced error", { err }); } });
                h2();
            } else {
                // Fallback: direct acquisition
                try { if (hasComponentType(item)) { compTypeValue = item.componentType; } } catch (err) { _localLogger.warn("Silenced error", { err }); }
            }
        }
    } catch (err) { _localLogger.warn("Silenced error", { err }); }
    return () => { for (const fn of unsubs) { try { fn(); } catch (err) { _localLogger.warn("Silenced error", { err }); } } };
});
...
onMount(() => {
    ...
    return () => { try { unsubscribe(); } catch (err) { _localLogger.warn("Silenced error", { err }); } };
});
...
function toggleComments() {
    if (hasOpenComment(generalStore)) {
        if (generalStore.openCommentItemId === model.id) {
            ...
            try { logger.debug(undefined, '[OutlinerItem] toggleComments id=' + model.id + ' -> false'); } catch (err) { _localLogger.warn("Silenced error", { err }); }
        } else {
            ...
            try { item.ensureComments?.(); } catch (err) { _localLogger.warn("Silenced error", { err }); }
            generalStore.openCommentItemId = model.id;
            try { logger.debug(undefined, '[OutlinerItem] toggleComments id=' + model.id + ' -> true index=' + index); } catch (err) { _localLogger.warn("Silenced error", { err }); }
        }
    }
}
...
function handleContentClick(e: MouseEvent) {
    ...
    if (btn) {
        try { logger.debug(undefined, '[OutlinerItem] handleContentClick toggling comments for id=' + model.id); } catch (err) { _localLogger.warn("Silenced error", { err }); }
        e.stopPropagation();
        toggleComments();
    }
}
...
async function handleDrop(event: DragEvent | CustomEvent) {
    ...
    if (maybeCustom?.detail && typeof maybeCustom.detail === "object" && "targetItemId" in maybeCustom.detail) {
        ...
        event.preventDefault?.();
        try { event.stopPropagation?.(); (event as Event).stopImmediatePropagation?.(); } catch (err) { _localLogger.warn("Silenced error", { err }); }
    ...
    try {
        ...
        if (import.meta.env.MODE === "test") {
            ...
            if (!(anyWin as E2EWindow).__E2E_FORCE_HANDLE_DROP__) {
                (anyWin as E2EWindow).__E2E_FORCE_HANDLE_DROP__ = (el: Element) => { try { selfInvoker(el); } catch (err) { _localLogger.warn("Silenced error", { err }); } };
            } else {

                const prev = (anyWin as E2EWindow).__E2E_FORCE_HANDLE_DROP__;
                (anyWin as E2EWindow).__E2E_FORCE_HANDLE_DROP__ = (el: Element) => { try { prev?.(el); } catch (err) { _localLogger.warn("Silenced error", { err }); } ; try { selfInvoker(el); } catch (err) { _localLogger.warn("Silenced error", { err }); } };
            }
            ...
            const selfAdd = (el: Element, text?: string) => {
                try {
                    if (displayRef && (el === displayRef || displayRef.contains(el))) {
                        ...
                        try { if (IS_TEST) { window.dispatchEvent(new CustomEvent('item-attachments-changed', { detail: { id: String(model.id) } })); } } catch (err) { _localLogger.warn("Silenced error", { err }); }
                    }
                } catch (err) { _localLogger.warn("Silenced error", { err }); }
            };
            if (!(anyWin as E2EWindow).__E2E_ADD_ATTACHMENT__) {
                (anyWin as E2EWindow).__E2E_ADD_ATTACHMENT__ = (el: Element, text?: string) => { try { selfAdd(el, text); } catch (err) { _localLogger.warn("Silenced error", { err }); } };
            } else {

                const prevAdd = (anyWin as E2EWindow).__E2E_ADD_ATTACHMENT__;
                (anyWin as E2EWindow).__E2E_ADD_ATTACHMENT__ = (el: Element, text?: string) => { try { prevAdd?.(el, text); } catch (err) { _localLogger.warn("Silenced error", { err }); }; try { selfAdd(el, text); } catch (err) { _localLogger.warn("Silenced error", { err }); } };
            }
        }
...
onMount(() => {
    ...
    try { document.addEventListener('drop', handler, true); } catch (err) { _localLogger.warn("Silenced error", { err }); }
    return () => { try { document.removeEventListener('drop', handler, true); } catch (err) { _localLogger.warn("Silenced error", { err }); }; };
});
...
>
        ...
        <div class="item-content-container">
            ...
            >
                ...
                <span
                    ...
                    oninput={(e) => { try { const t = (e.currentTarget as HTMLElement)?.textContent ?? ""; if ('updateText' in model.original && typeof model.original.updateText === 'function') { model.original.updateText(t); } } catch (err) { _localLogger.warn("Silenced error", { err }); } }}
                    onchange={(e) => { try { const t = (e.currentTarget as HTMLElement)?.textContent ?? ""; if ('updateText' in model.original && typeof model.original.updateText === 'function') { model.original.updateText(t); } } catch (err) { _localLogger.warn("Silenced error", { err }); } }}
                >
    ...
    {#if isCommentsVisible}
        ...
        <!-- eslint-disable-next-line svelte/no-at-html-tags -->
        { (() => { try { void item.comments; } catch (err) { _localLogger.warn("Silenced error", { err }); } return ''; })() }
        <CommentThread
