<script lang="ts">
    ...
    interface Props {
        ...
        currentUser?: string;
        onPageSelected?: (
            event: CustomEvent<{ pageId: string; pageName: string }>,
        ) => void;
    }
    ...
    let pageTitle = $state(
        isDev ? `New Page ${new Date().toLocaleTimeString()}` : "",
    );

    function handleCreatePage() {
        if (!project) {
            console.error("PageList: project is null, cannot create page");
            return;
        }
            ...
            <li>
                ...
                >
                    ...
                    <span class="page-date"
                        >{new Date(page.lastChanged).toLocaleDateString()}</span
                    >
                </button>
            </li>
        {/each}
