                ...
                try {
                    if (!e || (e.keysChanged && e.keysChanged.has && e.keysChanged.has('aliasTargetId'))) {
                        ...
                        if (newValue !== aliasTargetId) {
                            ...
                            (logger as any).debug("OutlinerItemAlias: aliasTargetId updated via observe", { itemId: modelId, newValue });
                        }
                    }
                } catch {}
            };
...
const aliasTarget = $derived.by(() => {
    try {
        ...
        (logger as any).warn({ error: e as Error }, "OutlinerItemAlias: alias target resolve error");
    }
    return undefined;
});
...
const aliasPath = $derived.by(() => {
    ...
    try {
        ...
        (logger as any).warn({ error: e as Error }, "OutlinerItemAlias: alias path resolve error");
    }
    return [] as Item[];
});
