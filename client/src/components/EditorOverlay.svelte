...
let overlayCursorVisible = $derived.by(() => {
    const isTestEnvironment = typeof window !== 'undefined' && (window.__E2E__ === true || window.navigator?.webdriver === true);
...
onMount(() => {
    ...
    try {
        ...
        if (typeof window !== 'undefined' && (window.__E2E__ === true || window.navigator?.webdriver === true)) {
            updatePositionMap();
...
function handleCopy(event: ClipboardEvent) {
  ...
  const boxSel = selections.find((s: SelectionRange) => s.isBoxSelection && s.boxSelectionRanges && s.boxSelectionRanges.length > 0);
  if (!selectedText && boxSel) {
    ...
    if (rectText) {
      ...
      if (typeof navigator !== 'undefined' && navigator.clipboard?.writeText) {
        navigator.clipboard.writeText(rectText).catch(() => {});
      }
      if (typeof window !== 'undefined') {
...
function handlePaste(event: ClipboardEvent) {
  ...
  if (typeof window !== 'undefined' && window.DEBUG_MODE) {
    ...
    const anySelectionUpdating = $derived.by(() => {
        try {
            return Object.values(store.selections).some((s: SelectionRange) => !!s?.isUpdating);
        } catch {
            return false;
        }
    });
...
<div class="editor-overlay" bind:this={overlayRef} class:paused={store.animationPaused} class:visible={overlayCursorVisible || localCursorVisible || (typeof window !== 'undefined' && (window.__E2E__ === true || window.navigator?.webdriver === true))} data-test-env={(typeof window !== 'undefined' && (window.__E2E__ === true || window.navigator?.webdriver === true)) ? 'true' : 'false'}>
    <!-- デバッグボタン -->
    <button
        class="debug-button"
    ...
    {#if !aliasPickerStore.isVisible || typeof window === 'undefined' || (window.__E2E__ === true || window.navigator?.webdriver === true)}
    <!-- 選択範囲のレンダリング -->
    ...
    {#if typeof window !== 'undefined' && (window.__E2E__ === true || window.navigator?.webdriver === true) && cursorList.length === 0}
        <div
            class="cursor"
