    ...
    const keydownHandler = (e: KeyboardEvent) => {
        if ((e.ctrlKey || e.metaKey) && (e.key === 'c' || e.key === 'C')) {
            try {
                ...
                if (!txt) {
                    const sels = Object.values(store.selections || {}) as unknown[];
...
onMount(() => {
    const handler = (e: CustomEvent) => {
        const open = !!(e?.detail as unknown)?.visible;
        try {
            if (open) {
...
function handleCopy(event: ClipboardEvent) {
  ...
  const boxSel = selections.find((s: unknown) => s.isBoxSelection && s.boxSelectionRanges && s.boxSelectionRanges.length > 0) as unknown;
  if (!selectedText && boxSel) {
    ...
    if (rectText) {
      ...
      if (typeof navigator !== 'undefined' && (navigator as unknown).clipboard?.writeText) {
        (navigator as unknown).clipboard.writeText(rectText).catch(() => {});
      }
      if (typeof window !== 'undefined') {
        window.lastCopiedText = rectText;
      }
      if (clipboardRef) {
  ...
  if (selectedText) {
    ...
    if (event.clipboardData) {
      ...
      if (cursorInstances.length > 1) {
        ...
        // 各カーソルの選択テキストを収集
        cursorInstances.forEach((cursor: unknown) => {
          const itemId = cursor.itemId;

          // 該当するアイテムの選択範囲を探す
          const selection = Object.values(store.selections).find((sel: unknown) =>
  ...
  if (selections.length === 1 && selections[0].startItemId === selections[0].endItemId) {
    ...
    if (typeof navigator !== 'undefined' && (navigator as unknown).clipboard?.writeText) {
      (navigator as unknown).clipboard.writeText(selectedText).catch(() => {});
    }
  ...
  if (combinedText) {
    ...
    // navigator.clipboard にも書き込み（Playwright 互換のため）
    if (typeof navigator !== 'undefined' && (navigator as unknown).clipboard?.writeText) {
      (navigator as unknown).clipboard.writeText(combinedText).catch(() => {});
    }
    // グローバル変数に保存（E2E テスト環境専用）
    // 本番環境では使用されないが、E2E テストでコピー内容を検証するために必要
    if (typeof window !== 'undefined') {
      window.lastCopiedText = combinedText;
    }
  }
...
function handlePaste(event: ClipboardEvent) {
  ...
  if (target && (target.id === 'clipboard-test' || target.closest?.('#clipboard-test'))) {
    ...
    const fallbackText = (typeof window !== 'undefined' && window.lastCopiedText) || '';
  ...
  if (typeof window !== 'undefined' && window.DEBUG_MODE) {
    ...
    const anySelectionUpdating = $derived.by(() => {
        try {
            return Object.values(store.selections).some((s: unknown) => !!s?.isUpdating);
        } catch {
            return false;
        }
    });
...
<div class="editor-overlay" bind:this={overlayRef} class:paused={store.animationPaused} class:visible={overlayCursorVisible || localCursorVisible || (typeof window !== 'undefined' && window.navigator?.webdriver)} data-test-env={(typeof window !== 'undefined' && window.navigator?.webdriver) ? 'true' : 'false'}>
    <!-- デバッグボタン -->
    <button
        class="debug-button"
    ...
    {#if !aliasPickerStore.isVisible || typeof window === 'undefined' || window.navigator?.webdriver}
    <!-- 選択範囲のレンダリング -->
    ...
    {#each cursorList as cursor (cursor.cursorId)}
        {@const isTestEnvironment = typeof window !== 'undefined' && window.navigator && window.navigator.webdriver}
    ...
    {#if typeof window !== 'undefined' && window.navigator?.webdriver && cursorList.length === 0}
        <div
            class="cursor"
