<script lang="ts">
    ...
    // Yjs minimal granularity observe: Monitor the underlying Y.Map("orderedTree")
    ...
    // Re-binding on Y.Doc switch is not needed: Stabilized by re-mounting OutlinerBase and this component with {#key}

    // E2E environment fallback: Ensure DOM update in environments where observe rarely fails to arrive
    onMount(() => {
        try {
    ...
    let displayItems = $derived.by<DisplayItem[]>(() => {
        // Dependency: Recompute every time __displayItemsTick updates
    ...
    // Re-measure height in response to change in number of visible items ($effect not used)
    // Legacy hook assuming update trigger by observeDeep (__displayItemsTick)
    ...
    // Add empty sibling item during editing of bottom item
    function handleEdit() {
    ...
    function handleNavigateToItem(event: CustomEvent) {
        ...
        const { direction, cursorScreenX, fromItemId, toItemId, shiftKey } =
            event.detail;
        // Clear existing selection for navigation without Shift (move to non-multi selection)
        if (!shiftKey) {
        ...
        if (toItemId) {
            ...
            if (direction === "up") {
                ...
                // If direction is not specified (e.g. alias path click)
        ...
        if (direction === "left") {
            ...
            if (currentIndex > 0) {
                ...
                // Stay on current item if first item
            ...
            // Do nothing if last item (move to end)
            focusItemWithPosition(
                fromItemId,
                Number.MAX_SAFE_INTEGER,
                shiftKey,
        ...
        // Shift+Down multi-selection: Get first range from store selections and update end
        ...
        // Shift+Up multi-selection: Get first range from store selections and update start
    ...
    function focusItemWithPosition(
        ...
        // If there is currently a focused item, finish editing
        ...
        if (activeItem && activeItem !== itemId) {
            // If there is a currently edited item and it differs from target
            const activeElement = document.querySelector(
                `[data-item-id="${activeItem}"]`,
            );
            if (activeElement) {
                // Finish editing on current item
    ...
    function handleAddSibling(event: CustomEvent) {
        ...
        if (currentIndex >= 0) {
            ...
            if (parent) {
                // If parent item exists, add after current item
    ...
    function handlePasteMultiItem(event: CustomEvent) {
        ...
        // If there are selections, delete selections then paste
    ...
    function handleMultiItemSelectionPaste(selection: any, lines: string[]) {
        ...
        // Remove items within selection (remove from back)
        for (let i = actualEndIndex; i >= actualStartIndex; i--) {
            if (i === actualStartIndex) {
                // Update text instead of removing start item
    ...
    function handleSingleItemSelectionPaste(selection: any, lines: string[]) {
        ...
        if (lines.length === 1) {
            // If single line paste, replace selection
            const newText =
            ...
            for (let i = 1; i < lines.length; i++) {
                ...
                if (newItem) {
                    if (i === lines.length - 1) {
                        // Last line adds text after selection
                        const lastItemText =
    ...
    // Mouse down event handler for entire tree
    function handleTreeMouseDown(event: MouseEvent) {
        ...
        // Ignore clicks inside items (processed by OutlinerItem)
    ...
    // Mouse up event handler for entire tree
    ...
    function updateDragSelection() {
        ...
        // Get indices of start and current item
        const startIndex = displayItems.findIndex(
        ...
        // Determine start and end of selection range
    ...
    function handleItemDrop(event: CustomEvent) {
        ...
        // If selection exists, remove selection then drop
        if (selection) {
            // Process removal of selection
            const startItemId = selection.startItemId;
            const endItemId = selection.endItemId;
    ...
    function handleSingleItemSelectionDrop(
        ...
        // Get indices of source and target items
        ...
        if (position === "top") {
            ...
            // Insert in middle (calculate cursor position)
    ...
    function handleItemMoveDrop(
        ...
        // Do nothing if source and target are same
...
<style>
    ...
    .outliner {
        ...
        overflow: hidden;
        margin-bottom: 20px;
        height: calc(100vh - 40px); /* Subtract margin from browser height */
        display: flex;
    ...
    .tree-container {
        padding: 8px 16px;
        position: relative; /* Reference point for child absolute positioning */
    ...
    .item-container {
        ...
        width: auto;
        min-height: 36px; /* Set min height */
    }
    ...
    .overlay-container {
        ...
        right: 0;
        bottom: 0;
        pointer-events: none !important; /* Ensure clicks pass through to layer below */
        z-index: 100;
