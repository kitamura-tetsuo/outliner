...
onMount(() => {
    try {
        const yArr = (item as any)?.attachments as Y.Array<unknown> | undefined;
        const read = () => {
            try {
                const arr = (yArr?.toArray?.() ?? []);
                attachmentsMirror = arr.map(u => Array.isArray(u) ? (u as unknown[])[0] : u) as string[];
        ...
        if (yArr && typeof yArr.observe === 'function' && typeof yArr.unobserve === 'function') {
            ...
            attachmentsMirror = ((item as any)?.attachments?.toArray?.() ?? []).map(u => Array.isArray(u) ? (u as unknown[])[0] : u) as string[];
        }
    } catch {}
});
...
// テスト環境用のイベントリスナー
onMount(() => {
    const onAtt = (_e: CustomEvent<{ id?: string }>) => {
        try {
