...
let { pageItem, isReadOnly = false, isTemporary = false, onEdit }: Props = $props();
...
function handleCreatePage() {
    if (isTemporary && onEdit) {
        logger.info("Create page button clicked, triggering onEdit callback");
        onEdit();
    }
}
...
function initializePageState() {
    ...
    if (items && items.length > 0) {
        previousChildItems = Array.from({ length: items.length }, (_, i) => {
            const item = items[i];
...
$effect.root(() => {
    ...
    // 子アイテムの変更を検出
    const childItemsChanged = hasChildItemsChanged(currentChildItems, previousChildItems);

    // 変更があった場合のみ処理を実行
    if (textChanged || childItemsChanged) {
        logContentChange(textChanged, childItemsChanged, currentText, currentChildItems);
...
function hasChildItemsChanged(current: string[], previous: string[]): boolean {
    // 子アイテムの数が変わった場合
