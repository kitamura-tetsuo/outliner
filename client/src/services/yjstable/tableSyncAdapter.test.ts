...
describe("TableSyncAdapter", { timeout: 30000 }, () => {
    ...
    // Note: the older "only emits the result of the latest runQueryNow call"
    // test relied on monkey-patching an internal `executeQuery` method that
    // used to live on the adapter. Query execution now belongs to the shared
    // `executeGridQuery` helper (used by `GridQueryRunner`), so generation
    // filtering is exercised at the runner level in `gridQueryRunner.test.ts`
    // rather than here.
