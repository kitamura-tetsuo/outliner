...
export async function runCalendarQuery(
    ...
    try {
        ...
        for (let round = 0;; round++) {
            try {
                result = await executeQuery(pgSchema, selectSql, range);
                break;
            } catch (err) {
...
async function executeQuery(pgSchema: string, selectSql: string, range?: CalendarRange): Promise<TableQueryResult> {
