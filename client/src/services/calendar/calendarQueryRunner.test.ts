...
describe("runCalendarQuery", { timeout: 30000 }, () => {
    ...
    describe("session timezone injection (§6.5)", () => {
        it("sets the SQL session timezone for the calendar's own query", async () => {
            const projectId = "proj-calendar-tz-set";
            const { projectDoc } = seedProject(projectId);
            const pgSchema = projectSchemaName(projectId);
            const session = createTableEngineSession({ projectDoc, projectId, connect: localConnector });
            try {
                ...
                expect(outcome.error).toBeUndefined();
                expect(outcome.result?.rows).toEqual([{ tz: "Asia/Tokyo" }]);
            } finally {
                session.dispose();
            }
        });

        it("does not leak the timezone into a later query that runs without one (transaction-local)", async () => {
            const projectId = "proj-calendar-tz-local";
            const { projectDoc } = seedProject(projectId);
            const pgSchema = projectSchemaName(projectId);
            const session = createTableEngineSession({ projectDoc, projectId, connect: localConnector });
            try {
                await runCalendarQuery(session, pgSchema, "SELECT 1", undefined, "Asia/Tokyo");
                const outcome = await runCalendarQuery(
                    session,
                    pgSchema,
                    "SELECT current_setting('TimeZone') AS tz",
                ...
                expect(outcome.error).toBeUndefined();
                expect(outcome.result?.rows).not.toEqual([{ tz: "Asia/Tokyo" }]);
            } finally {
                session.dispose();
            }
        });

        it("buckets a date_trunc/::date grouping expression differently under different fixed timezones for the same instant", async () => {
            // The same UTC instant is 2026-08-01 in UTC but already 2026-08-02
            // in Tokyo (UTC+9) once local time passes 15:00 UTC. This is what
            // makes session-timezone parity matter for grouping: two viewers
            // of a calendar with different *effective* timezones would bucket
            // the same row into different days.
            const projectId = "proj-calendar-tz-bucket";
            const { projectDoc } = seedProject(projectId);
            const pgSchema = projectSchemaName(projectId);
            const session = createTableEngineSession({ projectDoc, projectId, connect: localConnector });
            try {
                const query = "SELECT '2026-08-01T20:00:00Z'::timestamptz::date AS bucket";
                const utc = await runCalendarQuery(session, pgSchema, query, undefined, "UTC");
                const tokyo = await runCalendarQuery(session, pgSchema, query, undefined, "Asia/Tokyo");
                expect(utc.result?.rows).toEqual([{ bucket: "2026-08-01" }]);
                expect(tokyo.result?.rows).toEqual([{ bucket: "2026-08-02" }]);
            } finally {
                session.dispose();
            }
        });
