...
export class YjsClient {
    ...
    static async connect(projectId: string, project: Project): Promise<YjsClient> {
        ...
        try {
            ...
            try {
                const meta = doc.getMap("metadata") as unknown;
    ...
    public get isContainerConnected(): boolean {
        try {
            ...
            const p: unknown = this._provider;
    ...
    getAllData() {
        ...
        const collect = (it: Items | unknown): unknown => {
            const arr: unknown[] = [];
            const len = (it as unknown).length ?? 0;
            for (let i = 0; i < len; i++) {
                const item = (it as unknown).at ? (it as unknown).at(i) : (it as unknown)[i];
                if (!item) continue;
                const node: unknown = {
                    id: item.id,
                ...
                if (children && ((children as unknown).length ?? 0) > 0) {
    ...
    public async createPage(pageName: string, lines: string[]): Promise<void> {
        ...
        (pageItem.text as unknown).insert?.(0, pageName);
        const pageChildren = pageItem.items as Items;
        for (const line of lines) {
            const item = pageChildren.addNode("user");
            (item.text as unknown).insert?.(0, line);
        }
    }

    public dispose() {
        try {
            (this._provider as unknown)?.destroy?.();
        } catch {}
        try {
            (this._doc as unknown)?.destroy?.();
        } catch {}
        try {
