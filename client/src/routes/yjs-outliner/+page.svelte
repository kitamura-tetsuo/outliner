...
onMount(async () => {
  ...
  if ((p.items as unknown as { length?: number })?.length === 0) {
    const pg = p.addPage("Untitled", "tester");
    while ((pg.items as unknown as { length?: number })?.length && (pg.items as unknown as { length?: number })?.length < 3) pg.items.addNode("tester");
  }
  pageItem = (p.items as unknown[])?.at ? (p.items as unknown[])?.at(0) : (p.items as unknown[])?.[0];
  generalStore.project = p as unknown;
