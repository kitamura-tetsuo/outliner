...
if (browser) {
    (window as { generalStore?: unknown; appStore?: unknown }).generalStore = (window as { generalStore?: unknown }).generalStore || appStore;
    (window as { appStore?: unknown }).appStore = (window as { appStore?: unknown }).appStore || appStore;
}
...
if (browser) {
    try {
        ...
        if (!(appStore as { project?: unknown }).project) {
            (appStore as { project?: unknown }).project = (Project as { createInstance: (title: string) => unknown }).createInstance(projectTitle);
        ...
        if (pageTitle && !(appStore as { currentPage?: unknown }).currentPage && (appStore as { project?: { items?: { length: number; at?: (i: number) => unknown; addNode?: (text: string) => { updateText?: (t: string) => void } } } }).project) {
            try {
                const itemsAny: { length: number; at?: (i: number) => unknown; addNode?: (text: string) => { updateText?: (t: string) => void } } = (appStore as { project?: { items?: unknown } }).project?.items;
                // 既存ページにタイトル一致があるかチェック
                let found: unknown = null;
                const len = itemsAny?.length ?? 0;
                for (let i = 0; i < len; i++) {
                    const p = itemsAny.at ? itemsAny.at(i) : (itemsAny as { [index: number]: unknown })[i];
                ...
                if (found) (appStore as { currentPage?: unknown }).currentPage = found;
            } catch {}
        }
    } catch {}
}
...
function ensureCurrentPageByRoute(pj: string, pg: string) {
    try {
        ...
        const gs: { project?: { items?: { length: number; at?: (i: number) => unknown; addNode?: (text: string) => { updateText?: (t: string) => void } } }; currentPage?: unknown } = appStore;
        if (!gs?.project) return;
        const items: { length: number; at?: (i: number) => unknown; addNode?: (text: string) => { updateText?: (t: string) => void } } = gs.project.items as { length: number; at?: (i: number) => unknown };
        let found: unknown = null;
        const len = items?.length ?? 0;
        for (let i = 0; i < len; i++) {
            const p = items.at ? items.at(i) : (items as { [index: number]: unknown })[i];
            const t = p?.text?.toString?.() ?? String(p?.text ?? "");
...
onMount(async () => {
    ...
    if (browser) {
        ...
        try {
            (window as { __E2E_LAYOUT_MOUNTED__?: boolean }).__E2E_LAYOUT_MOUNTED__ = true;
        ...
        if (!isE2e && "serviceWorker" in navigator) {
            navigator.serviceWorker.register("/service-worker.js", { scope: "/" })
                .then(reg => {
                    ...
                    if ("sync" in reg) {
                        (reg as { sync?: { register: (name: string) => Promise<void> } }).sync.register("sync-ops").catch((err: unknown) => {
        ...
        try {
            if (typeof window !== 'undefined') {
                ...
                if (!(window as { __E2E_DROP_PATCHED__?: boolean }).__E2E_DROP_PATCHED__) {
                    (window as { __E2E_DROP_PATCHED__?: boolean }).__E2E_DROP_PATCHED__ = true;
                    ...
                    const wrap = function(this: unknown, orig: (...args: unknown[]) => unknown, event: Event): boolean {
                        ...
                        try { if (event && event.type === 'drop') { (window as { __E2E_ATTEMPTED_DROP__?: boolean }).__E2E_ATTEMPTED_DROP__ = true; } } catch {}
                        try {
                            if (event && event.type === 'drop' && !(event instanceof DragEvent)) {
                                ...
                                try { Object.defineProperty(de, 'dataTransfer', { value: (event as { dataTransfer?: unknown }).dataTransfer, configurable: true }); } catch {}
                                try { (window as { __E2E_DROP_HANDLERS__?: Array<(target: unknown, event: Event) => void> }).__E2E_DROP_HANDLERS__?.forEach((fn: (target: unknown, event: Event) => void) => { try { fn(this, de); } catch {} }); } catch {}
                                return orig.call(this, de);
                            }
                        } catch {}
                        try { if (event && event.type === 'drop') { (window as { __E2E_DROP_HANDLERS__?: Array<(target: unknown, event: Event) => void> }).__E2E_DROP_HANDLERS__?.forEach((fn: (target: unknown, event: Event) => void) => { try { fn(this, event); } catch {} }); } } catch {}
                    ...
                    try {
                        const anyWin: { [key: string]: unknown } = window as unknown as { [key: string]: unknown };
                        ...
                        if (!anyWin.__E2E_FILE_CTOR_PATCHED__) {
                            ...
                            if (OrigFile) {
                                const Wrapped = new Proxy(OrigFile, {
                                    construct(target: (...args: unknown[]) => unknown, args: unknown[]) {
                            ...
                            if (OrigDT) {
                                const WrappedDT = new Proxy(OrigDT, {
                                    construct(target: (...args: unknown[]) => unknown, args: unknown[]) {
                                        ...
                                        try {
                                            const list = (dt as { items: { add: (data: unknown, type?: string) => void } }).items;
                                            if (list && typeof list.add === 'function' && !list.__e2eAddPatched) {
        ...
        try {
            window.addEventListener('drop', (ev: DragEvent) => {
                try { console.log('[GlobalDrop] drop received target=', (ev?.target as { className?: string; tagName?: string })?.className || (ev?.target as { className?: string; tagName?: string })?.tagName); } catch {}
            }, { capture: true });
            document.addEventListener('drop', (ev: Event) => {
                try { console.log('[DocDrop] drop received target=', (ev?.target as { className?: string; tagName?: string })?.className || (ev?.target as { className?: string; tagName?: string })?.tagName); } catch {}
            }, { capture: true });
            window.addEventListener('dragover', (ev: Event) => {
                try { console.log('[GlobalDrop] dragover received target=', (ev?.target as { className?: string; tagName?: string })?.className || (ev?.target as { className?: string; tagName?: string })?.tagName); } catch {}
            }, { capture: true });
        } catch {}
