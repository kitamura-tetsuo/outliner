<script lang="ts">
    ...
    import "$lib";
    import { cleanupFluidClient, initFluidClientWithAuth } from "../services";
                        ...
                        if (isTestEnv) {
                            ...
                            (async () => {
                                try {
                                    // ユーザーのコンテナリストを取得
                                    const { containers } =
                                        await fluidService.getUserContainers();
                                    ...
                                    for (const containerId of containers) {
                                        try {
                                            ...
                                            const success =
                                                await fluidService.deleteContainer(
                                                    containerId,
                                                );
                                    ...
                                    // 新しいテスト用コンテナを作成
                                    fluidService.createNewContainer("test-2");
                                    fluidService.createNewContainer("test-1");
                                } catch (error) {
                                    logger.error(
