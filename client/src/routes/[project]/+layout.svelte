...
$effect(() => {
    ...
    const projectParam = (pageStore?.params?.project as string) || (data as unknown)?.project;
    ...
    if (isTestEnv && !store.project) {
        try {
            ...
            project = provisional as unknown;
...
async function loadProject(projectNameFromParam?: string) {
    try {
        const projectName = projectNameFromParam ?? (data as unknown).project;
