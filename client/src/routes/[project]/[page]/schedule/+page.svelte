...
onMount(async () => {
    ...
    if (!pageId) {
        try {
            const items: unknown = store.pages?.current;
            const len = (items as { length?: number })?.length ?? 0;
            let found: unknown = undefined;
            for (let i = 0; i < len; i++) {
                const p = (items as unknown[])?.at ? (items as unknown[]).at(i) : (items as unknown[])?.[i];
                const title = p && typeof p === 'object' ? (p as { text?: unknown })?.text?.toString?.() ?? String((p as { text?: unknown })?.text ?? "") : "";
                if (String(title).toLowerCase() === String(pageTitle).toLowerCase()) {
                    found = p;
                    break;
                }
            }
            if (found) pageId = String((found as { id?: unknown })?.id ?? "");
        } catch {}
    }
