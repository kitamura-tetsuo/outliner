...
async function loadProjectAndPage() {
    ...
    try {
                ...
                if (proj) {
                    ...
                    try {
                        ...
                        if (Array.isArray(pendingImport) && pendingImport.length > 0) {
                            const projectItems: unknown = proj.items as unknown;
                    ...
                    try {
                        const itemsAny: unknown = (store.project as unknown).items as unknown;
                                        ...
                                        if (shouldReplaceChildren) {
                                            ...
                                            const cloneBranch = (sourceItems: unknown, targetItems: unknown) => {
                                                ...
                                                for (let index = 0; index < length; index++) {
                                                    ...
                                                    mapId((srcNode as unknown)?.id, (destNode as unknown)?.id);
                        ...
                        if (!shouldSkipTestSeed()) {
                            ...
                            if (isTestEnv && pageRef) {
                                const _pageItems: unknown = (pageRef as unknown).items as unknown;
                            ...
                            if (isTestEnv && !shouldSkipTestSeed()) {
                                ...
                                const trySeed = () => {
                                    try {
                                        ...
                                        const pageItems2: unknown = ref2?.items as unknown;
        ...
        if (store.project) {
            ...
            if (projectLooksLikePlaceholder(store.project)) {
                ...
                if (snapshot) {
                    ...
                    try {
                        const pages: unknown = hydrated.items as unknown;
        ...
        if (store.pages) {
            ...
            {
                const arr: unknown = store.pages.current as unknown;
                const len = arr?.length ?? 0;
                for (let i = 0; i < len; i++) {
                    const p = arr?.at ? arr.at(i) : arr[i];
                    const title = (p?.text as unknown)?.toString?.() ?? String((p as unknown)?.text ?? "");
    ...
    try {
        ...
        const iv = setInterval(() => {
            try {
                ...
                const pg: unknown = store.currentPage as unknown;
...
function capturePageIdForSchedule() {
    try {
        ...
        const pg: unknown = store.currentPage as unknown;
        if (!pg) return;
        const children: unknown = pg?.items as unknown;
...
function addItemFromTopToolbar() {
    try {
        let pageItem: unknown = store.currentPage as unknown;
        // currentPage が未用意なら、URL の pageName で暫定ページを作成
        if (!pageItem) {
            const proj: unknown = store.project as unknown;
