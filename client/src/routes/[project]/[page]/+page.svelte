...
function projectLooksLikePlaceholder(candidate: unknown): boolean {
    ...
    try {
        const items = (candidate as { items?: unknown }).items as unknown;
...
async function loadProjectAndPage() {
    ...
    try {
        if (!store.project) {
            ...
            if (typeof window !== "undefined") {
                logger.debug("DEBUG: provisional store.project set?", !!((window as any).generalStore?.project)); // eslint-disable-line @typescript-eslint/no-explicit-any
            }
    ...
    try {
                ...
                if (proj) {
                    ...
                    try {
                        ...
                        if (Array.isArray(pendingImport) && pendingImport.length > 0) {
                            const projectItems = proj.items as unknown;
                            ...
                            const populate = (nodes: unknown[], targetItems: unknown) => {
                                ...
                                for (const nodeData of nodes) {
                                    ...
                                    populate(children, node?.items as unknown);
                                }
                            };
                            for (const root of pendingImport) {
                                ...
                                populate(root?.children ?? [], pageNode?.items as unknown);
                            }
                        ...
                        if (!appliedPendingImport && snapshot && Array.isArray(snapshot.items) && snapshot.items.length > 0) {
                            ...
                            const populateChildren = (children: unknown[], targetItems: unknown) => {
                                ...
                                for (const child of children ?? []) {
                                    ...
                                    populateChildren(child?.children ?? [], node?.items as unknown);
                                }
                            };
                            ...
                            for (const root of snapshot.items) {
                                ...
                                populateChildren(root?.children ?? [], pageNode?.items as unknown);
                            }
                        }
                    } catch (snapshotError) {
                    ...
                    try {
                        const itemsAny = (store.project as unknown).items as unknown;
                                    ...
                                    for (let attempt = 0; attempt < 20; attempt++) {
                                        ...
                                        if (shouldReplaceChildren) {
                                            ...
                                            const copyAttachments = (sourceNode: unknown, targetNode: unknown) => {
                                                try {
                                                    const srcAtt = sourceNode?.attachments;
                                            ...
                                            const cloneBranch = (sourceItems: unknown, targetItems: unknown) => {
                                                ...
                                                for (let index = 0; index < length; index++) {
                                                    ...
                                                    mapId((srcNode as { id?: unknown })?.id, (destNode as { id?: unknown })?.id);
                                                    copyAttachments(srcNode, destNode);
                                                    cloneBranch(srcNode?.items as unknown, destNode?.items as unknown);
                                                }
                                            };
                                            ...
                                            cloneBranch(prev.items as unknown, next.items as unknown);
                                ...
                                if (pendingPage) {
                                    const applyChildren = (nodes: unknown[], targetItems: unknown) => {
                                        ...
                                        for (const nodeData of nodes ?? []) {
                                            ...
                                            applyChildren(nodeData?.children ?? [], child?.items as unknown);
                                        }
                                    };
                                    applyChildren(pendingPage?.children ?? [], pageRef?.items as unknown);
                        ...
                        if (!shouldSkipTestSeed()) {
                            ...
                            if (isTestEnv && pageRef) {
                                const _pageItems = pageRef?.items as unknown;
        ...
        // グローバルストアの状態をログ出力
        if (typeof window !== "undefined") {
            const globalStore = (window as any).generalStore; // eslint-disable-line @typescript-eslint/no-explicit-any
                            ...
                            if (isTestEnv && !shouldSkipTestSeed()) {
                                ...
                                const trySeed = () => {
                                    try {
                                        ...
                                        const pageItems2 = ref2?.items as unknown;
        ...
        if (store.project) {
            ...
            if (projectLooksLikePlaceholder(store.project)) {
                ...
                if (snapshot) {
                    ...
                    try {
                        const pages = hydrated.items as unknown;
        ...
        if (store.pages) {
            logger.info(`Available pages count: ${store.pages.current.length}`);
            {
                ...
                for (let i = 0; i < len; i++) {
                    ...
                    const title = (p?.text as { toString?: () => unknown })?.toString?.() ?? String((p as { text?: unknown })?.text ?? "");
        ...
        if (!shouldSkipTestSeed()) try {
            ...
            if (store.project && store.pages && isTestEnv) {
                ...
                const ensurePage = (title: string) => {
                    try {
                        if (typeof (store.project as unknown).addPage === "function") {
                            return (store.project as unknown).addPage(title, "tester");
...
function capturePageIdForSchedule() {
    try {
        ...
        const pg = store.currentPage as unknown;
        if (!pg) return;
        const children = pg?.items as unknown;
...
function addItemFromTopToolbar() {
    try {
        ...
        if (!pageItem) {
            const proj = store.project as unknown;
            if ((proj as { addPage?: unknown })?.addPage && pageName) {
                try {
                    const created = (proj as { addPage: (arg1: string, arg2: string) => unknown }).addPage(pageName, "tester");
                    if (created) {
...
function toggleSearchPanel() {
    ...
    if (typeof window !== "undefined") {
        (window as any).__SEARCH_PANEL_VISIBLE__ = isSearchPanelVisible; // eslint-disable-line @typescript-eslint/no-explicit-any
    }
...
onMount(async () => {
    ...
    if (typeof window !== "undefined") {
        (window as any).__OPEN_SEARCH__ = async () => { // eslint-disable-line @typescript-eslint/no-explicit-any
            ...
            (window as any).__SEARCH_PANEL_VISIBLE__ = true; // eslint-disable-line @typescript-eslint/no-explicit-any
