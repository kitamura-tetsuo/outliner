...
async function createNewContainer() {
    ...
    try {
        ...
        // 2. Save to server to confirm access rights (Important to do before WebSocket connection)
        logger.info(`Saving new container ID ${newProjectId} to server...`);
        const saveResult = await saveContainerId(newProjectId);
        if (!saveResult) {
            throw new Error("Failed to save container ID to server");
        }

        // 3. Start WebSocket connection after rights are confirmed
    ...
    {:else}
        <div class="mx-auto max-w-md rounded-lg bg-yellow-50 p-6 shadow-md">
            ...
            <p class="mb-4 text-gray-700">
                To create a new outliner, please login from the login button above first.
            </p>
        </div>
    {/if}
