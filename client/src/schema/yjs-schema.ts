...
// Type for item values in Y.Map
type ItemValue = string | number | Y.Text | Y.Array<string> | Y.Array<Y.Map<unknown>> | undefined;
...
export class Item {
    ...
    private get value(): Y.Map<ItemValue> {
        return this.tree.getNodeValueFromKey(this.key) as Y.Map<ItemValue>;
    }
...
export class Items {
    ...
    addNode(author: string, index?: number): Item {
        ...
        const value = new Y.Map<ItemValue>();
