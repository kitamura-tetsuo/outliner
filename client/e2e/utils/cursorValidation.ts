...
export class CursorValidator {
        ...
        if (Array.isArray(expected)) {
            ...
            for (let i = 0; i < expected.length; i++) {
                if (i < actual.length) {
                    ...
                    throw new Error(
                        `Expected array item at index ${i} not found in actual array`,
                    );
                }
            }
        } else {
    ...
    static async compareWithSnapshot(
        ...
        const filteredSnapshot = this.removeIgnoredPaths(
            JSON.parse(JSON.stringify(snapshot)),
            ignorePaths,
        );
        const filteredCurrent = this.removeIgnoredPaths(
            JSON.parse(JSON.stringify(currentData)),
            ignorePaths,
        );

        expect(JSON.stringify(filteredCurrent)).toBe(
     ...
     */
    static async assertCursorCount(
        page: Page,
     ...
     */
    static async assertActiveItemId(
        page: Page,
    ...
    static async getDOMCursorInfo(page: Page): Promise<{
        ...
        return await page.evaluate(() => {
            ...
            const activeCursors = Array.from(
                document.querySelectorAll(".cursor.active"),
            );
     ...
     */
    static async validateCursorState(
        page: Page,
        expectedCount: number,
     ...
     */
    static async assertActiveCursorCount(
        page: Page,
        expectedCount: number,
    ): Promise<void> {
        const domInfo = await this.getDOMCursorInfo(page);
     ...
     */
    static async assertCursorBlink(
        page: Page,
        waitTime: number = 600,
    ): Promise<void> {
