...
export class DataValidationHelpers {
    ...
    try {
      ...
      await page
        ...
        .catch((e) => {
          ...
          console.log(
            "[afterEach] cleanup evaluation skipped:",
            e?.message ?? e,
          );
        });
    } catch (e: unknown) {
  ...
  static async saveSnapshotsAndCompare(
    page: Page,
    ...
    if (page.isClosed()) {
      console.log(
        "[saveSnapshotsAndCompare] Page already closed, skipping snapshot",
      );
      return;
    }
    ...
    try {
      ...
      if (
        !errMsg.includes("Test ended.") &&
        !errMsg.includes("Target page, context or browser has been closed")
      ) {
        console.warn(
          "[saveSnapshotsAndCompare] waitForFunction failed:",
          errMsg,
        );
      }
    ...
    try {
      result = await page.evaluate(
        async () => {
          function yTextToString(t: any): string {
            if (!t) return "";
            try {
              return typeof t.toString === "function"
                ? t.toString()
                : String(t);
            } catch {
              return String(t);
          ...
          if (!store || !store.project) {
            throw new Error(
              "Project not available on globalThis.generalStore/appStore",
            );
          }
          ...
          for (let i = 0; i < len; i++) {
            ...
            pages.push({
              title: pageTitle,
              items: [{ text: pageTitle }, ...children],
            });
          }

          return {
            mode: "yjs",
            yjsJson: JSON.stringify(
              { projectTitle: String(project.title ?? ""), pages },
              null,
              2,
            ),
          };
      ...
      console.warn(
        "[saveSnapshotsAndCompare] evaluate failed:",
        e?.message ?? e,
      );
