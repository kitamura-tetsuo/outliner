    ...
    public static async setAccessibleProjects(
        ...
        try {
            await page.waitForFunction(
                () =>
                    ...
                    && !!(globalThis as any).__PROJECT_STORE__,
                { timeout: 15000 },
            );
        ...
        for (const projectId of projectNames) {
            ...
            if (existing.status === 404) {
                ...
                throw new Error(`setAccessibleProjects: descriptor lookup failed (${existing.status})`);
            }
        }
                            ...
                            Element.prototype.getAttribute = function(name) {
                                try {
                                    if (name === "data-alias-target-id") {
                                        ...
                                        if (
                                            ap?.lastConfirmedItemId && String(itemId) === String(ap.lastConfirmedItemId)
                                        ) {
