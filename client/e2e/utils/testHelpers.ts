...
export class TestHelpers {
  ...
  private static readonly LOG_ENABLED: boolean =
    process.env.E2E_VERBOSE_SLOG === "1";
  private static slog(msg: string, data?: unknown) {
    if (!TestHelpers.LOG_ENABLED) return;
    const now = Date.now();
    const delta =
      TestHelpers.__lastLogTs == null ? 0 : now - TestHelpers.__lastLogTs;
   ...
   */
  public static async stopCoverage(
    page: Page,
    testName: string,
  ): Promise<void> {
  ...
  public static async seedProjectAndNavigate(
    ...
    const workerIndex =
      typeof testInfo?.workerIndex === "number" ? testInfo.workerIndex : 1;
    const timestamp = Date.now();
    const projectName =
      options?.projectName ?? `Test Project ${workerIndex} ${timestamp}`;
    ...
    if (!options?.doNotSeed && !options?.skipSeed) {
      try {
        ...
        TestHelpers.slog(
          `seedProjectAndNavigate: SeedClient seeding failed: ${e}`,
        );
      }
    }
    ...
    if (!options?.doNotNavigate) {
      ...
      try {
        ...
        TestHelpers.slog(
          "Warning: __YJS_STORE__ not connected within timeout",
          { error: e?.message },
        );
      }
      ...
      try {
        ...
        TestHelpers.slog(
          "Warning: generalStore not initialized within timeout",
          { error: e?.message },
        );
      }
      ...
      if (!options?.skipAppReady) {
        ...
        await expect(page.getByTestId("outliner-base")).toBeVisible({
          timeout: 15000,
        });
  ...
  public static async login(
    ...
    await page.evaluate(
      async ({ e, p }) => {
        ...
        if (um && typeof um.loginWithEmailPassword === "function") {
          ...
          console.error(
            "[E2E] UserManager or loginWithEmailPassword not found",
          );
    ...
    await page.waitForFunction(
      () => !!(globalThis as any).__USER_MANAGER__?.auth?.currentUser,
      {
        timeout: 15 * 1000,
      },
    );
  ...
  public static async setAccessibleProjects(
    ...
    try {
      await page.waitForFunction(
        () =>
          ...
          !!(globalThis as any).__USER_MANAGER__,
        { timeout: 15000 },
      );
      ...
      if (!hasUser) {
        TestHelpers.slog(
          "setAccessibleProjects: currentUser is null, attempting manual login...",
        );
      ...
      console.error(
        "setAccessibleProjects: CRITICAL: Could not establish authentication.",
        e,
      );
    ...
    const currentUserId = await page.evaluate(
      () => (globalThis as any).__USER_MANAGER__?.auth?.currentUser?.uid,
    );
    TestHelpers.slog(
    ...
    await page.evaluate(
      async ({ projects }) => {
        ...
        if (ps && typeof ps.syncFromFirestore === "function") {
          ...
          console.log(
            "[setAccessibleProjects] Triggered projectStore.syncFromFirestore()",
          );
        } else {
          console.log(
            "[setAccessibleProjects] Warning: projectStore not found for sync",
          );
        }
        ...
        if (fs && fs.testSaveUserProject) {
          ...
          console.log(
            "[setAccessibleProjects] Project saved to Firestore emulator",
          );
        }
      },
    ...
    TestHelpers.slog(
      "[setAccessibleProjects] Seeding complete, waiting 500ms for propagation...",
    );
  ...
  public static async seedProjectDataOnly(
    ...
    const workerIndex =
      typeof testInfo?.workerIndex === "number" ? testInfo.workerIndex : 1;
    const projectName =
      options?.projectName ?? `Test Project ${workerIndex} ${Date.now()}`;
      ...
      try {
        await page.addInitScript(() => {
          try {
            ...
            localStorage.setItem("VITE_FIREBASE_PROJECT_ID", "outliner-d57b0");
            localStorage.setItem("VITE_YJS_DEBUG", "true"); // ENABLE DEBUG
            (globalThis as unknown as Window & Record<string, any>).__E2E__ =
              true;
              ...
              Element.prototype.getAttribute = function (name) {
                try {
                  if (name === "data-alias-target-id") {
                    ...
                    if (
                      ap?.lastConfirmedItemId &&
                      String(itemId) === String(ap.lastConfirmedItemId)
                    ) {
                      return ap?.lastConfirmedTargetId != null
                        ? String(ap.lastConfirmedTargetId)
                        : "";
                    }
                  }
                } catch {}
        ...
        TestHelpers.slog(
          `seedProjectDataOnly: Failed to set localStorage flags via addInitScript: ${e}`,
        );
      }
    }
    ...
    if (!options?.skipSeed) {
      try {
        ...
        TestHelpers.slog(
          `seedProjectDataOnly: SeedClient seeding failed: ${e}`,
        );
  ...
  public static async navigateToProjectPage(
    ...
    console.log(
      `[DEBUG] navigateToProjectPage: Start navigating to ${projectName} / ${pageName}`,
    );
    const encodedProject = encodeURIComponent(projectName);
    const encodedPage = encodeURIComponent(pageName);
    const url = `/${encodedProject}/${encodedPage}?isTest=true`;
    ...
    await page
      ...
      .catch(() =>
        TestHelpers.slog("Warning: __YJS_STORE__ connect wait timed out"),
      );
    TestHelpers.slog("__YJS_STORE__ connected");
    ...
    if (page.isClosed()) {
      TestHelpers.slog(
        "Page closed during YJS connect wait, aborting navigation",
      );
      return;
    }
    await page.waitForTimeout(1000);
    ...
    await page
      ...
      .catch(async (e) => {
        ...
        TestHelpers.slog(
          "Warning: currentPage not set or mismatch within timeout",
          { error: e?.message },
        );
    ...
    if (page.isClosed()) {
      TestHelpers.slog(
        "Page closed during waitForAppReady, aborting navigation",
      );
      return;
    }
    ...
    TestHelpers.slog("Waiting for outliner items to render...", {
  ...
  public static async seedProjectAndNavigateForProject(
    ...
    // Always seed to create the page, even if lines is empty
    {
      try {
        ...
        TestHelpers.slog(
          `seedProjectAndNavigateForProject: SeedClient seeding failed: ${e}`,
        );
      }
    }
    ...
    console.log("TestHelper: Project environment ready", {
      projectName,
      pageName,
    });
    return { projectName, pageName };
   ...
   */
  public static async getPageTexts(
    page: Page,
  ): Promise<Array<{ id: string; text: string }>> {
    return await page.evaluate(() => {
      const store =
        (globalThis as unknown as Window & Record<string, any>).appStore ||
        ...
        try {
          ...
          if (typeof len === "number" && len >= 0) {
            ...
            for (let i = 0; i < len; i++) {
              ...
              const v =
                typeof anyVal.at === "function" ? anyVal.at(i) : anyVal[i];
      ...
      return pages.map((p: any) => {
        const textVal =
          p?.text &&
          typeof (p.text as { toString?: unknown }).toString === "function"
  ...
  private static async setupCursorDebugger(page: Page): Promise<void> {
    try {
      await page.evaluate(() => {
        ...
        globalThis.getCursorDebugData = function () {
          ...
          try {
            ...
            editorOverlayStore.cursorInstances.forEach(
              (cursor: any, id: string) => {
      ...
      console.log(
        "TestHelper: setupCursorDebugger injection skipped:",
        (e as any)?.message ?? e,
      );
    }
    ...
    try {
      await page.evaluate(() => {
        ...
        const buildYjsSnapshot = () => {
          try {
            const gs =
              (globalThis as any).generalStore || (globalThis as any).appStore;
            ...
            const toPlain = (item: any): any => {
              ...
              const textVal =
                item.text?.toString?.() ?? String(item.text ?? "");
      ...
      console.log(
        "TestHelper: setupTreeDebugger injection skipped:",
        (e as any)?.message ?? e,
      );
    }
  }
   ...
   */
  public static async waitForCursorVisible(
    page: Page,
    timeout = 15000,
  ): Promise<boolean> {
    ...
    try {
      ...
      if (page.isClosed()) {
        TestHelpers.slog(
          "waitForCursorVisible: page closed during wait, returning false",
        );
        return false;
      }
      console.log(
        "Timeout waiting for cursor to be visible, continuing anyway",
      );
      void _error;
      ...
      try {
        if (!page.isClosed()) {
          await page.screenshot({
            path: "client/test-results/cursor-visible-timeout.png",
          });
        }
   ...
   * @param timeout Timeout in milliseconds
   */
  public static async waitForItemCount(
    page: Page,
   ...
   * @param timeout Timeout in milliseconds
   */
  public static async ensureCursorReady(
    page: Page,
    timeout = 10000,
  ): Promise<void> {
    ...
    await page
      .waitForFunction(
        () => {
          const textarea =
            document.querySelector<HTMLTextAreaElement>(".global-textarea");
  ...
  public static async setCursor(
    ...
    await page
      ...
      .catch(() => {
        throw new Error(
          "TestHelpers.setCursor: editorOverlayStore.setCursor is not available within timeout",
        );
      });

    const setSucceeded = await page.evaluate(
      ({ itemId, offset, userId }) => {
        const editorOverlayStore = (globalThis as any).editorOverlayStore;
        if (
          editorOverlayStore &&
          typeof editorOverlayStore.setCursor === "function"
        ) {
          console.log(
        ...
        console.error(
          `TestHelpers.setCursor: editorOverlayStore or setCursor not available`,
        );
        return false;
      },
      { itemId, offset, userId },
    ...
    if (!setSucceeded) {
      throw new Error(
        `TestHelpers.setCursor: failed to set cursor for itemId=${itemId}`,
      );
    }
  }
  ...
  public static async insertText(
    ...
    await page.evaluate(
      async ({ itemId, text, userId }) => {
        const editorOverlayStore = (globalThis as any).editorOverlayStore;
        if (editorOverlayStore && editorOverlayStore.getCursorInstances) {
          const cursorInstances = editorOverlayStore.getCursorInstances();
          const cursor = cursorInstances.find(
            (c: any) => c.itemId === itemId && c.userId === userId,
          );
          if (cursor && cursor.insertText) {
            console.log(
              `TestHelpers.insertText: Found cursor for itemId=${itemId}, userId=${userId}`,
            );
            cursor.insertText(text);
          } else {
            console.error(
              `TestHelpers.insertText: Cursor not found for itemId=${itemId}, userId=${userId}`,
            );
            console.log(
              `Available cursors:`,
              cursorInstances.map((c: any) => ({
                itemId: c.itemId,
                userId: c.userId,
              })),
            );
          ...
          console.error(
            `TestHelpers.insertText: editorOverlayStore or getCursorInstances not available`,
          );
        }
      },
      { itemId, text, userId },
   ...
   */
  public static async waitForAppReady(
    page: Page,
    skipAuthCheck = false,
  ): Promise<void> {
    ...
    await page.waitForFunction(() => !!(globalThis as any).generalStore, {
      timeout: 60000,
    });
      ...
      await page
        ...
        .catch(async (e) => {
          ...
          await page.evaluate(() => {
            ...
            console.log(
              "YJS.isConnected:",
              (globalThis as any).__YJS_STORE__?.isConnected,
            );
          });
          throw e;
        });
   ...
   */
  public static async waitForPageData(
    page: Page,
    pageName: string,
    timeout = 30000,
  ): Promise<void> {
    ...
    while (Date.now() < deadline) {
      ...
      try {
          ...
          try {
            ...
            if (!gs.currentPage) {
              ...
              if (pages && typeof pages.current?.id === "string") {
                return {
                  ready: false,
                  reason: "currentPage not set but project has pages",
                };
              }
            ...
            const pageText =
              ...
              String(currentPage?.text ?? "");
            if (
              !pageText ||
              pageText.toLowerCase() !== targetPageName.toLowerCase()
            ) {
              return {
                ready: false,
        ...
        if (hasData.ready) {
          TestHelpers.slog("waitForPageData: success", {
            pageName,
            itemCount: hasData.itemCount,
          });
          return;
        }

        TestHelpers.slog("waitForPageData: waiting", {
          pageName,
          reason: hasData.reason,
          details: hasData,
        });
        consecutiveErrors = 0;
      } catch (e) {
        ...
        if (consecutiveErrors > 5) {
          ...
          try {
            const debugInfo = await page.evaluate(() => {
              return {
                ...
                generalStoreKeys: (globalThis as any).generalStore
                  ? Object.keys((globalThis as any).generalStore).filter(
                      (k) => !k.startsWith("_"),
                    )
                  : "no gs",
            ...
            TestHelpers.slog("waitForPageData: debug info", {
              pageName,
              debug: debugInfo,
            });
          } catch {}
    ...
    throw new Error(
      `Timeout waiting for page data: ${pageName} after ${timeout}ms`,
    );
  }
   ...
   */
  public static async waitForOutlinerItems(
    page: Page,
    count = 1,
    timeout = 30000,
  ): Promise<void> {
    ...
    console.log(
      `Waiting for ${count} outliner items (with data-item-id) to be visible...`,
    );
    ...
    try {
      ...
      if (!yjsConnected) {
        TestHelpers.slog(
          "waitForOutlinerItems: Warning - Yjs not connected yet",
        );
      }
      ...
      while (Date.now() < deadline) {
        ...
        try {
          ...
          const countNow = await page
            .locator(".outliner-item[data-item-id]")
            .count();
      ...
      if (!ensured) {
        ...
        if (finalCount >= minRequiredItems) {
          ...
          TestHelpers.slog(
            "waitForOutlinerItems: Items not found, retrying with extended timeout",
            {
              finalCount,
              minRequiredItems,
            },
          ...
          while (Date.now() < extendedDeadline && !page.isClosed()) {
            ...
            if (retryCount >= minRequiredItems) {
              TestHelpers.slog(
                "waitForOutlinerItems: Items found after extended wait",
                {
                  retryCount,
                  minRequiredItems,
                },
      ...
      TestHelpers.slog("waitForOutlinerItems: success", {
        itemCount,
        minRequiredItems,
      });
    } catch (e) {
   ...
   * @param timeout Timeout in milliseconds
   */
  public static async waitForPagesList(
    page: Page,
    timeout = 15000,
  ): Promise<void> {
  ...
  public static async getActiveItemLocator(page: Page): Promise<any> {
    ...
    return page.locator(
      `.outliner-item[data-item-id="${activeItemId}"] .item-content`,
    );
  }
   ...
   */
  public static async getItemIdByIndex(
    page: Page,
    index: number,
  ): Promise<string | null> {
    let attempts = 0;
   ...
   */
  public static async clickItemToEdit(
    page: Page,
    itemSelector: string,
  ): Promise<void> {
   ...
   */
  public static async forceHoverEvent(
    page: Page,
    selector: string,
  ): Promise<void> {
   ...
   * @param selector Selector for target element
   */
  public static async forceMouseOutEvent(
    page: Page,
    selector: string,
  ): Promise<void> {
    await page.evaluate((sel) => {
  ...
  public static async openBacklinkPanel(page: Page): Promise<void> {
    ...
    const isOpen = await toggleButton.evaluate((el) =>
   ...
   */
  public static async confirmAliasOption(
    page: Page,
    itemId: string,
  ): Promise<void> {
  ...
  public static async selectAliasOption(
    page: Page,
    itemId: string,
  ): Promise<void> {
    // Ensure picker is visible first
    await page
      .locator(".alias-picker")
      .first()
  ...
  public static async clickAliasOptionViaDOM(
    page: Page,
    itemId: string,
  ): Promise<void> {
  ...
  public static async setAliasTarget(
    page: Page,
    itemId: string,
    targetId: string,
  ): Promise<void> {
    ...
    await page
      .locator(".alias-picker")
      .first()
      .locator(`button[data-id="${targetId}"]`)
      .waitFor({
        state: "visible",
        timeout: 5000,
      });
  ...
  public static async hideAliasPicker(page: Page): Promise<void> {
    ...
    if (isVisible) {
      try {
        ...
        console.log(
          "Failed to hide alias picker with Escape, trying alternative method",
        );
  ...
  public static async showAliasPicker(
    page: Page,
    itemId: string,
  ): Promise<void> {
   ...
   */
  public static async getAliasTargetId(
    page: Page,
    itemId: string,
  ): Promise<string | null> {
        ...
        if (node) {
          ...
          if (!value) {
            try {
              ...
              const ymap: any = anyItem?.tree?.getNodeValueFromKey?.(
                anyItem?.key,
              );
   ...
   */
  public static async isAliasPathVisible(
    page: Page,
    itemId: string,
  ): Promise<boolean> {
    ...
    try {
      const attr = await page
        .locator(itemSel)
        .getAttribute("data-alias-target-id");
   ...
   */
  public static async isAliasSubtreeVisible(
    page: Page,
    itemId: string,
  ): Promise<boolean> {
    const itemSel = `.outliner-item[data-item-id="${itemId}"]`;
   ...
   */
  public static async clickAliasPathButton(
    page: Page,
    itemId: string,
    buttonIndex: number,
  ): Promise<void> {
    const aliasPath = page.locator(
      `.outliner-item[data-item-id="${itemId}"] .alias-path`,
    );
   ...
   */
  public static async getAliasPathButtonCount(
    page: Page,
    itemId: string,
  ): Promise<number> {
    const aliasPath = page.locator(
   ...
   */
  public static async getAliasTarget(
    page: Page,
    itemId: string,
  ): Promise<string | null> {
   ...
   * @returns Item text
   */
  public static async getTextByItemId(
    page: Page,
    itemId: string,
  ): Promise<string> {
  ...
  public static async getBoxSelectionText(
    ...
    for (const range of boxSelectionRanges) {
      ...
      const startOffset = Math.max(
        0,
        Math.min(fullText.length, Math.min(range.startOffset, range.endOffset)),
      );
      const endOffset = Math.max(
        0,
        Math.min(fullText.length, Math.max(range.startOffset, range.endOffset)),
      );
  ...
  public static async forceCheckVisibility(
    ...
    for (let i = 0; i < retryCount; i++) {
      try {
        ...
        if (!elementExists) {
          console.log(
            `Element not found: ${selector} (attempt ${i + 1}/${retryCount})`,
          );

          // For internal links, try to force rendering
          if (
            selector.includes(".internal-link") ||
            selector.includes(".link-preview")
          ) {
            ...
            await page.evaluate(() => {
              ...
              console.log(
                `Found ${textElements.length} text elements to check for links`,
              );
        ...
        console.log(
          `Element found but not visible: ${selector} (attempt ${i + 1}/${retryCount})`,
        );
  ...
  public static async cleanup(page: Page): Promise<void> {
    try {
      ...
      if (
        ...
        console.log(
          "TestHelper cleanup: page context no longer available, skipping",
        );
      } else {
  ...
  public static async getTestAuthToken(): Promise<string> {
    ...
    const authHost =
      process.env.VITE_FIREBASE_AUTH_EMULATOR_HOST || "localhost:59099";
    // Ensure protocol
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (text.includes("EMAIL_NOT_FOUND")) {
          console.log(
            "[TestHelpers] User not found, creating new test user...",
          );
          try {
            ...
            if (
              ...
              console.log(
                "[TestHelpers] Race condition detected (EMAIL_EXISTS). Retrying sign-in...",
              );
              // Retry sign-in
          ...
          console.error(
            `[TestHelpers] Auth failed: ${response.status} ${text}`,
          );
      ...
      console.log(
        "[TestHelpers] Auth fetch failed, generating offline mock token.",
      );
