...
test.describe("MEM-0001: Member Management Flow", () => {
    ...
    test.beforeEach(async ({ page }) => {
        // Create test projects and set up initial sharing states
        await TestHelpers.login(page, OWNER_MGR_EMAIL, OWNER_MGR_PASSWORD);

        // Create test projects if they don't exist
        await TestHelpers.createTestProjectIfNotExists(page, PROJECT_ROLE_CHANGE_TEST);
        await TestHelpers.createTestProjectIfNotExists(page, PROJECT_MEMBER_REMOVAL_TEST);
        await TestHelpers.createTestProjectIfNotExists(page, PROJECT_NON_OWNER_TEST);

        // Don't logout - keep the owner logged in for the tests
        // await TestHelpers.logout(page);
    });

    test("Update Member Role (Editor to Viewer)", async ({ page }) => {
        // Navigate to the project (already logged in as owner from beforeEach)
        await TestHelpers.navigateToProject(page, PROJECT_ROLE_CHANGE_TEST);

        // Wait for the page to load and check if Manage Members button is visible
        await page.waitForTimeout(2000);

        // Check if user has owner role by looking for the Manage Members button
        const manageMembersButton = page.getByRole("button", { name: "Manage Members" });
        ...
        test.info().annotations.push({
            type: "info",
            description: 'Viewer "add item" action restriction verified by checking item count does not change.',
        });
    ...
    test("Remove Member from Project", async ({ page }) => {
        ...
        const memberRow = membersModal.locator("div.flex", { has: page.locator(`p:text("${TO_BE_REMOVED_EMAIL}")`) })
            .first();
        await expect(memberRow).toBeVisible();
        ...
        if (await containerSelector.isVisible({ timeout: 5000 })) {
            ...
            test.info().annotations.push({
                type: "info",
                description: "ContainerSelector not found on / to verify project absence. Navigating directly.",
            });
        }
        ...
        test.info().annotations.push({
            type: "info",
            description: "Access denial verified by checking URL redirection.",
        });
    });
