...
test.describe("Multi-Page Schedule Management", () => {
    ...
    test.beforeEach(async ({ page }, testInfo) => {
        ...
        // Create project with both pages upfront using seedProjectDataOnly
        testProject = await TestHelpers.seedProjectDataOnly(
            page,
            testInfo,
            [
    ...
    test.afterEach(async ({ page }) => {
        ...
        try {
            ...
            if (page.isClosed()) {
                console.log(
                    "[afterEach] Page already closed, skipping schedule cleanup",
                );
                return;
            }
    ...
    test("schedule operations across pages", async ({ page }) => {
            ...
            await page.waitForFunction(
                () => {
                    ...
                    if (client.getProject) {
                        const cp = typeof client.getProject === "function"
                            ? client.getProject()
                            : client.getProject;
        ...
        await expect(
            items.first().getByText(`${newTime.replace("T", " ")}`),
