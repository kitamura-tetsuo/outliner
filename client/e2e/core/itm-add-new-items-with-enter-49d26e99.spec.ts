...
test.describe("ITM-0001: Add new item with Enter", () => {
  ...
  test("Pressing Enter key splits the item at the cursor position", async ({
    page,
  }) => {
    ...
    const activeItem = page.locator(
      `.outliner-item[data-item-id="${activeItemId}"]`,
    );
  ...
  test("Text before the cursor position remains in the current item", async ({
    page,
  }) => {
    // Wait for cursor to be visible
    await TestHelpers.waitForCursorVisible(page);
    ...
    // Get active item
    const activeItem = page.locator(
      `.outliner-item[data-item-id="${firstItemId}"]`,
    );
    await activeItem.waitFor({ state: "visible" });

    // Get initial text and calculate cursor position
    const preInitialText = await page
      .locator(`.outliner-item[data-item-id="${firstItemId}"]`)
      .locator(".item-text")
      .textContent();
  ...
  test("Text after the cursor position moves to the new item", async ({
    page,
  }) => {
    // Wait for cursor to be visible
    await TestHelpers.waitForCursorVisible(page);
    ...
    // Get active item
    const newActiveItem = page.locator(
      `.outliner-item[data-item-id="${secondItemId}"]`,
    );
    await newActiveItem.waitFor({ state: "visible" });

    // Get text of the second item
    const secondItemText = await page
      .locator(`.outliner-item[data-item-id="${secondItemId}"]`)
      .locator(".item-text")
      .textContent();
  ...
  test("Cursor moves to the beginning of the new item", async ({ page }) => {
    ...
    const animationPaused = await cursor.isVisible();
