...
test.describe("Consistency of format display when moving cursors", () => {
  ...
  test("Control characters display/hide appropriately when moving cursor", async ({
    page,
  }) => {
    // Setup test page
    ...
    await expect
      .poll(
        async () => {
          ...
          return (
            html.includes("internal-link") || html.includes("control-char")
          );
        },
        { timeout: 2000 },
      )
    ...
    const secondItemHtmlInactive = await secondItem
      .locator(".item-text")
      .innerHTML();
    ...
    const firstItemHtmlActive = await firstItem
      .locator(".item-text")
      .innerHTML();
    ...
    const secondItemHtmlActive = await secondItem
      .locator(".item-text")
      .innerHTML();
    expect(secondItemHtmlActive).toContain(
      '<span class="control-char">[</span>asd',
    );
  });
  ...
  test("Title is displayed as normal text", async ({ page }) => {
    ...
    const titleClasses = await pageTitle
      .locator(".item-text")
      .getAttribute("class");
    expect(titleClasses).toContain("title-text");

    // Check title CSS style
    const titleFontWeight = await pageTitle
      .locator(".item-text")
      .evaluate((el) => {
    ...
    const titleTextWithoutCursor = await pageTitle
      .locator(".item-text")
      .textContent();
  ...
  test("External link syntax is displayed correctly", async ({ page }) => {
    ...
    expect(JSON.stringify(treeDataAfterLink)).toContain(
      "[https://example.com]",
    );
    const firstItemTextContentInactive = await firstItem
      .locator(".item-text")
      .textContent();
    ...
    const firstItemTextContentActive = await firstItem
      .locator(".item-text")
      .textContent();
  ...
  test("Internal link syntax is displayed correctly", async ({ page }) => {
    ...
    await page.keyboard.press("Control+a");
    await page.keyboard.press("Backspace");
    const firstItemIdForInternalLink = await TestHelpers.getItemIdByIndex(
      page,
      0,
    );
    ...
    expect(
      pageTextsAfterInternalLink.some(
        ({ text }) => text === "[asd]" || text?.includes("[asd]"),
      ),
    ).toBe(true);
    await page.waitForTimeout(300);
    const firstItemTextContentInactiveInternal = await firstItem
      .locator(".item-text")
      .textContent();
    ...
    // Get text content of the first item (confirm control characters are displayed)
    const firstItemTextContentActiveInternal = await firstItem
      .locator(".item-text")
      .textContent();
  ...
  test("SharedTree data is saved correctly", async ({ page }) => {
    ...
    const hasFormattedText = itemsArray.some(
      (item: any) => item.text === "[[aasdd]]",
    );
...
test.afterEach(async ({ page }) => {
  ...
  await page
    ...
    .catch((error) => {
      console.warn(
        "Warning: Failed to reset editor store in afterEach:",
        error,
      );
    });
