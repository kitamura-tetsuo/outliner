...
test.describe("CLM-b8389849: Click outside text on the last line", () => {
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    await page
      .waitForSelector(".outliner-item[data-item-id]", { timeout: 30000 })
      .catch(() => {
        console.log(
          "Warning: Outliner items with data-item-id not found within timeout",
        );
      });
  });

  test("Cursor appears at the end of the line when clicking outside text on the last line", async ({
    page,
  }) => {
    const longText = "A".repeat(80);
    ...
    await page.screenshot({
      path: "client/test-results/CLM-0001-last-line-start.png",
    });
    ...
    const animationPaused = await TestHelpers.waitForCursorVisible(page, 30000);
    console.log("Cursor visible for last line test:", animationPaused);
    ...
    const visualLineYs = await activeItem
      .locator(".item-text")
      .evaluate((el) => {
    ...
    const textRect = await targetItem
      .locator(".item-text")
    ...
    // Wait for cursor to be visible
    const lastLineCursorVisible = await TestHelpers.waitForCursorVisible(
      page,
      5000,
    );
    ...
    // Wait for cursor to be visible
    const finalCursorVisible = await TestHelpers.waitForCursorVisible(
      page,
      30000,
    );
    ...
    const actualTextContent = await activeItem
      .locator(".item-text")
      .textContent();
    ...
    expect(finalCursorData.cursors[0].offset).toBe(
