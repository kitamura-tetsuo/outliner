...
test.describe("Admin Check (API-0003)", () => {
    ...
    test("Returns authentication error with invalid token", async ({ page }) => {
        ...
        try {
            const healthResponse = await page.request.get("http://127.0.0.1:57070/api/health");
        ...
        let response = await page.request.post("http://127.0.0.1:57070/api/adminCheckForProjectUserListing", {
    ...
    test("Returns 400 when projectId is not specified", async ({ page }) => {
        ...
        const response = await page.request.post("http://127.0.0.1:57070/api/adminCheckForProjectUserListing", {
