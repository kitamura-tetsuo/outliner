...
test.describe("ATT-0001: Drag and drop attachments", () => {
    ...
    test("attachment preview appears", async ({ page }) => {
        ...
        await page.waitForFunction(() => !!(globalThis as any).__E2E_ADD_ATTACHMENT__, null, { timeout: 5000 });
        ...
        await page.evaluate(() =>
            new Promise<void>(resolve => {
                const handler = () => {
                    try {
                        globalThis.removeEventListener("item-attachments-changed", handler as any);
                    } catch {}
                    resolve();
                };
                globalThis.addEventListener("item-attachments-changed", handler as any, { once: true } as any);
    ...
    test("multiple attachments can be added to same item", async ({ page }) => {
        ...
        await page.evaluate(() => {
            const el = document.querySelector(".outliner-item .item-content") as Element | null;
            if (!el) throw new Error("item-content not found");
            (globalThis as any).__E2E_ADD_ATTACHMENT__?.(el, "a");
