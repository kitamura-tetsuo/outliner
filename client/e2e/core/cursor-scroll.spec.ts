...
test.describe("Cursor scrolling behavior", () => {
    test("Cursor stays visible when moving down through a very tall item", async ({ page }, testInfo) => {
        ...
        // Add a long text item that spans multiple heights of the viewport (reduced from 150 to 50 for CI speed)
        ...
        // Insert text (using type so it processes Enter keys natively if applicable)
        ...
        // Move cursor down 35 times (should scroll the window down to follow the cursor)
        for (let i = 0; i < 35; i++) {
