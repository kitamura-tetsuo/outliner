...
test.describe("SLR-0010: Change selection format", () => {
  test.beforeEach(async ({ page }, testInfo) => {
    ...
    expect(cursorData.animationPaused).toBe(true);
  });

  test("Can change selection within a single item to bold", async ({
    page,
  }) => {
    ...
    await page
      ...
      .catch(async () => {
        const actual = await page.evaluate(() =>
          (globalThis as any).editorOverlayStore?.getSelectedText(),
        );
        console.log(
          `Timed out waiting for editorOverlayStore. Actual: "${actual}"`,
        );
      });
  ...
  test("Can change selection within a single item to italic", async ({
    page,
  }) => {
    // Click and select the first item
    const firstItem = page.locator(".outliner-item").nth(1);
  ...
  test("Can add underline to selection within a single item", async ({
    page,
  }) => {
    // Click and select the first item
    const firstItem = page.locator(".outliner-item").nth(1);
    ...
    const afterFormatInnerHTML = await firstItem
      .locator(".item-text")
      .innerHTML();
    const afterFormatTextContent = await firstItem
      .locator(".item-text")
      .textContent();
  ...
  test("Can change selection within a single item to strikethrough", async ({
    page,
  }) => {
    // Click and select the first item
    const firstItem = page.locator(".outliner-item").nth(1);
  ...
  test("Can change selection within a single item to code", async ({
    page,
  }) => {
    // Click and select the first item
    const firstItem = page.locator(".outliner-item").nth(1);
  ...
  test("Can change format of selection spanning multiple items", async ({
    page,
  }) => {
    // Click and select the first item
    const firstItem = page.locator(".outliner-item").nth(1);
    ...
    expect(firstItemHtml).toContain(
      "<strong>This is a test text for formatting</strong>",
    );
    ...
    expect(secondItemHtml).toContain(
      "is another line for multi-item selection",
    );
  });
});
