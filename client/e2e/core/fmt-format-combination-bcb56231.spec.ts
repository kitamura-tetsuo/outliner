...
test.describe("Format Combination", () => {
    ...
    test("Combination of bold and italic is displayed correctly", async ({ page }) => {
        ...
        await TestHelpers.prepareTestEnvironment(page, test.info(), [
            "This is a [[combination of bold and [/ italic]]] text",
        ]);
        ...
        expect(firstItemHtml).toContain("combination of bold and");
        expect(firstItemHtml).toContain("<em>");
        expect(firstItemHtml).toContain("italic");
        expect(firstItemHtml).toContain("</em>");
        expect(firstItemHtml).toContain("text");
        expect(firstItemHtml).toContain("</strong>");
    });
    ...
    test("Combination of bold and strikethrough is displayed correctly", async ({ page }) => {
        // Create data using the lines parameter of prepareTestEnvironment
        await TestHelpers.prepareTestEnvironment(page, test.info(), [
            "This is a [[combination of bold and [-strikethrough]]] text",
        ]);
    ...
    test("Combination of italic and code is displayed correctly", async ({ page }) => {
        // Create data using the lines parameter of prepareTestEnvironment
        await TestHelpers.prepareTestEnvironment(page, test.info(), [
            "This is a [/ combination of italic and `code`] text",
        ]);
        ...
        expect(firstItemHtml).toContain("<em>");
        expect(firstItemHtml).toContain("combination of italic and");
    ...
    test("Nested multiple formats are displayed correctly", async ({ page }) => {
        ...
        // Verify that complex combinations are displayed correctly
        expect(firstItemHtml).toContain("<strong>");
    ...
    test("Combined formats are displayed as plain text in the item with the cursor", async ({ page }) => {
        ...
        // Select the second item (the first item that is not the page title)
