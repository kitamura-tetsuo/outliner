...
test.describe("CLM-0003: Move to the right", () => {
    ...
    test("ArrowRight key moves the cursor one character to the right", async ({ page }) => {
        ...
        // Verify that the cursor is at the left end
        ...
        // Verify that the cursor offset has changed by one character (moved to the right)
        expect(newOffset).toBe(initialOffset + 1);

        // Verify that the number of cursors remains one
    ...
    test("When at the last character, moves to the first character of the next item", async ({ page }) => {
        ...
        console.log(`Number of items at start of test: ${initialItemCount}`);
        ...
        // Verify that the second item exists (Item 2)
        ...
        // Verify the text content of the second item
        ...
        // Get cursor information and verify it actually moved to the end
        let cursorData = await CursorValidator.getCursorData(page);
        ...
        // Verify: Is the cursor actually at the end? (Does text length match offset?)
        ...
        console.log(`Re-verification: Offset=${confirmedOffset}, Text length=${initialText?.length}`);
        ...
        // Get ID of the second item (already retrieved)
        ...
        // Type text into the second item and verify it is entered correctly
        await page.keyboard.type("Test input");

        // Re-verify the text content of the second item
