...
test.describe("CMT-0001: comment threads", () => {
  test.beforeEach(async ({ page }, testInfo) => {
    await TestHelpers.seedProjectAndNavigate(page, testInfo, ["first line"]);
    ...
    try {
      ...
      if (page.isClosed()) {
        console.log(
          "[beforeEach] Page already closed, skipping comment cleanup",
        );
        return;
      }
                ...
                try {
                  ...
                  if (
                    ...
                    for (const comment of comments) {
                      if (
                        comment.id &&
                        typeof item.deleteComment === "function"
                      ) {
  ...
  test("add, edit and remove comment", async ({ page }) => {
    ...
    await expect(
      page.locator(
        `[data-item-id="${firstId}"] [data-testid="comment-thread"]`,
      ),
    ).toBeVisible({
      timeout: 30000,
    });
    ...
    await expect(
      page.locator('[data-testid="comment-thread"] .text'),
    ).toContainText("hello", { timeout: 30000 });
    ...
    await expect(
      page.locator(`[data-testid="edit-input-${commentId}"]`),
    ...
    await expect(
      page.locator(`[data-testid="comment-${commentId}"] .text`),
    ).toHaveText("edited", {
      timeout: 60000,
    });
  ...
  test.afterEach(async ({ page }) => {
    ...
    try {
      ...
      if (page.isClosed()) {
        console.log(
          "[afterEach] Page already closed, skipping comment cleanup",
        );
        return;
      }
