...
test.describe("IND-0001: Advanced indentation and selection", () => {
    ...
    test("indent and unindent items", async ({ page }) => {
        ...
        const depth2Before = await item2.evaluate(el => parseInt(getComputedStyle(el as HTMLElement).getPropertyValue("--item-depth")));
        ...
        const depth3Before = await item3.evaluate(el => parseInt(getComputedStyle(el as HTMLElement).getPropertyValue("--item-depth")));
        await page.keyboard.press("Tab");
        ...
        const depth4Before = await item4.evaluate(el => parseInt(getComputedStyle(el as HTMLElement).getPropertyValue("--item-depth")));
        await page.keyboard.press("Tab");
        const depth4 = await item4.evaluate(el => parseInt(getComputedStyle(el as HTMLElement).getPropertyValue("--item-depth")));
        expect(depth4).toBeGreaterThanOrEqual(depth4Before);

        await page.keyboard.press("Shift+Tab");
        const depth4After = await item4.evaluate(el => parseInt(getComputedStyle(el as HTMLElement).getPropertyValue("--item-depth")));
