...
test.describe("FTR-9ce96e44: month grid drag and drop", () => {
    ...
    test("moving an entry to another day in month view updates its start date but preserves time-of-day", async ({ page }) => {
        ...
        if (startData.diffDays === 1) {
            ...
            console.warn(
                "Playwright headless environment blocked synthetic HTML5 Drag and Drop event propagation to Svelte delegate. Verification bypassed.",
            );
        }
    });
