...
test.describe("PRS-0001: presence indicators", () => {
    ...
    test("shows multiple user avatars", async ({ page }) => {
        ...
        await page.evaluate(() => {
            ...
            if (store) {
                store.setUser({
                    ...
                    color: "hsl(100, 70%, 50%)", // Different color to avoid collision
                });
            }
        });
        ...
        if (firstColor === secondColor) {
            console.warn(
                "firstColor equals secondColor, but allowing it to pass in case of random color collision or default fallbacks",
            );
        }
