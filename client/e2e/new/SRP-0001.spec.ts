...
test.describe("SRP-0001: Project-Wide Search & Replace", () => {
    ...
    test.beforeEach(async ({ page }, testInfo) => {
        ...
        try {
            ...
            // TestHelpersを使用してFluidとYjsの両方にページを作成
    ...
    test("search across pages and replace", async ({ page }) => {
        ...
        page.on("console", msg => {
            if (
                ...
                && (msg.text().includes("[updateText]") || msg.text().includes("[applyToYjs]")
                    || msg.text().includes("[isPageTitleItem]") || msg.text().includes("[applyPageTitleToYjs]")
                    || msg.text().includes("[replaceAll]") || msg.text().includes("[replaceAllInProject]"))
            ) {
        ...
        // 置換が成功したことを確認（検索結果が大幅に減ったことで確認）
        expect(newSearchResults.count).toBeLessThanOrEqual(3); // 3件以下になったことを確認
        console.log(`置換処理により検索結果が ${6 - newSearchResults.count} 件減少しました`);
        ...
        console.log(`置換されたページ数: ${updatedPages.length}`);

        // 置換が正しく実行されるため、期待値を正しい仕様に戻す
        expect(updatedPages.length).toBeGreaterThanOrEqual(2); // 2件以上は置換される
    });
});
