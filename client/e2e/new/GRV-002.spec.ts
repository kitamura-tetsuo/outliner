    ...
    test("graph updates when new page is added", async ({ page }) => {
        ...
        const pageCreateResult = await page.evaluate(async () => {
            try {
                ...
                await fluidStore.fluidClient.createPage("test-link", ["second page content"]);

                // Yjs統合: 並行してYjsページを作成
                try {
                    const yjsProjectManager = (window as any).__YJS_PROJECT_MANAGER__;
                    if (yjsProjectManager) {
                        console.log(`GRV-002: Creating Yjs page: "test-link"`);
                        await yjsProjectManager.createPage("test-link", "test-user", ["second page content"]);
                        console.log(`GRV-002: Yjs page created successfully: "test-link"`);
                    } else {
                        console.warn(`GRV-002: YjsProjectManager not available for page creation`);
                    }
                } catch (yjsError) {
                    // Yjsエラーは警告として記録するが、Fluidの処理は継続
                    console.warn(`GRV-002: Failed to create Yjs page: ${yjsError}`);
                }
