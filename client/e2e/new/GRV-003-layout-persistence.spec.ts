...
test.describe("GRV-0002: Graph view layout persistence", () => {
    test.afterEach(async ({ page }) => {
        // FluidとYjsのデータ整合性を確認（グラフビューページではプロジェクトタイトルチェックをスキップ）
        await DataValidationHelpers.validateDataConsistency(page, {
            checkProjectTitle: false, // グラフビューページではプロジェクトタイトルが取得できない場合があるためスキップ
            checkPageCount: false, // グラフビューページではページ数チェックもスキップ
            checkPageTitles: false, // グラフビューページではページタイトルチェックもスキップ
            checkItemCounts: false, // グラフビューページではアイテム数チェックもスキップ
        });
    });
