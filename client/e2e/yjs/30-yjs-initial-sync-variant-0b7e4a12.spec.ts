...
test("initial sync on late join (p1 connect -> update -> p2 connect)", async ({ browser }) => {
    ...
    await p1.waitForFunction(() => !!(window as any).__USER_MANAGER__?.getCurrentUser?.(), null, { timeout: 20000 });
    ...
    await p2.waitForFunction(() => !!(window as any).__USER_MANAGER__?.getCurrentUser?.(), null, { timeout: 20000 });
