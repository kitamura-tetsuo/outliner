 ...
 * the execution counts of event handlers and callbacks are not converted correctly.
 *
 * Solution:
 * Extract function execution counts from the raw V8 coverage data and
 ...
 *
 * Prerequisites:
 *   - V8 coverage data must exist in coverage/e2e/raw/
 *   - Istanbul format coverage data must exist in coverage/e2e/coverage-final.json
 */
...
// Support running from both client/scripts and workspace
...
console.log("Starting correction of function execution counts from V8 coverage to Istanbul format...");
...
if (!fs.existsSync(istanbulCoverageFile)) {
    ...
    console.error(`Looked in path: ${istanbulCoverageFile}`);
    process.exit(1);
}
...
// Get normalized file name from URL
...
for (const file of coverageFiles) {
    ...
    for (const entry of data) {
        ...
        for (const func of entry.functions || []) {
            ...
            // In V8 coverage, the function execution count is recorded in the count of the first range
...
// Read Istanbul format coverage data
...
console.log("\n✓ Function execution counts correction completed");

// Display list of fixed functions
if (totalFixed > 0) {
    console.log("\nFixed functions:");
    for (const [filePath, coverage] of Object.entries(istanbulCoverage)) {
        const fileName = path.basename(filePath);
        ...
        for (const [fnId, fnInfo] of Object.entries(fnMap)) {
            ...
            if (v8Count !== undefined && v8Count > 0 && f[fnId] === v8Count) {
                console.log(`  - ${fileName}:${fnInfo.loc.start.line} ${functionName} (${v8Count} times)`);
            }
        }
    }
}
