 ...
 * execution counts of event handlers and callbacks are not converted correctly.
 *
 * Solution:
 * Extract function execution counts from V8 coverage raw data,
 * and reflect them in Istanbul format coverage data.
 *
 * Usage:
...
// Handle both execution from client/scripts and workspace
...
console.log("Starting to fix function execution counts from V8 coverage to Istanbul format...");
...
if (!fs.existsSync(istanbulCoverageFile)) {
    ...
    console.error(`Path searched: ${istanbulCoverageFile}`);
    process.exit(1);
}
...
console.log(`  ✓ ${coverageFiles.length} V8 coverage files found`);
...
for (const file of coverageFiles) {
    ...
    for (const entry of data) {
        ...
        for (const func of entry.functions || []) {
            ...
            // In V8 coverage, function execution count is recorded in the first range count
