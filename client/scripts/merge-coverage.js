 ...
 *
 * Merges coverage from unit/integration/e2e tests
...
console.log("Starting coverage report integration...");
...
// Vitest integrates coverage of all projects into one file
if (fs.existsSync(unitAndIntegrationCoverageFile)) {
    ...
    console.log(`  ✓ ${files.length} files coverage added`);
    ...
    console.log(`     Path searched: ${unitAndIntegrationCoverageFile}`);
}
...
if (fs.existsSync(e2eCoverageFile)) {
    ...
    if (jsFiles.length === 0 && cssFiles.length > 0) {
        ...
        console.error("   This may be due to an issue during E2E coverage generation.");
        console.error("\n   Check the following:");
        console.error("   1. scripts/generate-e2e-coverage.js entryFilter settings");
        console.error("   2. scripts/generate-e2e-coverage.js sourceFilter settings");
        console.error("   3. monocart-coverage-reports version");
        process.exit(1);
    }
} else {
    console.log("  ⚠ E2E test coverage not found");
    console.log(`     Path searched: ${e2eCoverageFile}`);
}

if (filesFound === 0) {
    console.error("\nError: Coverage file not found");
    console.error("First, run tests with the following commands:");
...
// Save integrated coverage
console.log("\nSaving integrated coverage...");
...
console.log("\n✓ Coverage report integration completed");
