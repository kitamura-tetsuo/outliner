...
describe("Cloud Functions", () => {
    ...
    describe("exportSchedules", () => {
        it("should generate an iCal file with the correct content", async () => {
            ...
            const snapshot = {
                forEach: (callback) => {
                    callback({ id: "1", data: () => ({ strategy: "test-strategy", nextRunAt: { toDate: () => new Date() } }) });
                },
            };
            ...
            expect(res.setHeader).toHaveBeenCalledWith("Content-Disposition", "attachment; filename=\"schedules.ics\"");
