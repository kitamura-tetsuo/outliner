...
exports.permanentlyDeleteProject =
  require("./src/permanentlyDeleteProject").permanentlyDeleteProject;
...
exports.verifyPublicAccessToken = onRequest(
  { cors: true },
  async (req, res) => {
    setCorsHeaders(req, res);
    if (req.method === "OPTIONS") {
    ...
    try {
      ...
      logger.error(`Error verifying public access token: ${error.message}`, {
        error,
      });
      return res.status(500).json({
        error: "Failed to verify public access token",
      });
    }
  },
);
