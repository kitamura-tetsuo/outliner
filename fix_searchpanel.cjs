...
content = content.replace(
    /\(window as any\)\.__SEARCH_PANEL_VISIBLE__/g,
    "(window as Window & typeof globalThis & { __SEARCH_PANEL_VISIBLE__?: boolean }).__SEARCH_PANEL_VISIBLE__",
);
...
content = content.replace(
    /typeof \(items as any\)\.length === "number"/g,
    'items && typeof (items as unknown as { length: number }).length === "number"',
);
content = content.replace(
    /const len = \(items as any\)\.length;/g,
    "const len = (items as unknown as { length: number }).length;",
);
content = content.replace(
    /const v = \(items as any\)\.at\s*\?\s*\(items as any\)\.at\(i\)\s*:\s*\(items as any\)\[i\];/g,
    "const v = (items as unknown as { at?: (i: number) => Item })?.at ? (items as unknown as { at: (i: number) => Item }).at(i) : (items as unknown as Item[])[i];",
);

content = content.replace(
    /const gs = \(window as any\)\.generalStore;/g,
    "const gs = (window as Window & typeof globalThis & { generalStore?: any }).generalStore;",
);
content = content.replace(/for \(const p of pages as any\)/g, "for (const p of pages as unknown as Iterable<Item>)");
content = content.replace(
    /typeof \(pages as any\)\.length === "number"/g,
    'pages && typeof (pages as unknown as { length: number }).length === "number"',
);
content = content.replace(
    /const len = \(pages as any\)\.length;/g,
    "const len = (pages as unknown as { length: number }).length;",
);
content = content.replace(
    /const v = \(pages as any\)\.at\s*\?\s*\(pages as any\)\.at\(i\)\s*:\s*\(pages as any\)\[i\];/g,
...
content = content.replace(
    /const pageTitle = \(\(p as any\)\.text\?\.toString\?\.\(\) \?\?\s*String\(\(p as any\)\.text \?\? ""\)\) as string;/g,
...
content = content.replace(
    /len = \(children as any\)\?\.length \?\? 0;/g,
    "len = (children as unknown as { length?: number })?.length ?? 0;",
);
content = content.replace(
    /const child = \(children as any\)\.at\s*\?\s*\(children as any\)\.at\(i\)\s*:\s*\(children as any\)\[i\];/g,
...
content = content.replace(
    /\(window as any\)\.__E2E_LAST_MATCH_COUNT__/g,
    "(window as Window & typeof globalThis & { __E2E_LAST_MATCH_COUNT__?: number }).__E2E_LAST_MATCH_COUNT__",
);

content = content.replace(
    /\(m\.page as any\)\?\.text\?\.toString\?\.\(\) \?\?\s*String\(\(m\.page as any\)\?\.text \?\? ""\)/g,
    'm.page?.text?.toString?.() ?? String(m.page?.text ?? "")',
);
content = content.replace(
    /\(m\.item as any\)\?\.text\?\.toString\?\.\(\) \?\?\s*String\(\(m\.item as any\)\?\.text \?\? ""\)/g,
...
content = content.replace(
    /const text = \(\(p as any\)\.text\?\.toString\?\.\(\) \?\?\s*String\(\(p as any\)\.text \?\? ""\)\) as string;/g,
...
content = content.replace(
    /replaceFirst\(p as any, searchQuery, replaceText, options\)/g,
    "replaceFirst(p, searchQuery, replaceText, options)",
);
content = content.replace(/p as any,/g, "p,");

content = content.replace(
    /if \(newText !== text && \(p as any\)\.updateText\)\s*\(p as any\)\.updateText\(newText\);/g,
    'if (newText !== text && "updateText" in p && typeof p.updateText === "function") {\n                        p.updateText(newText);\n                    }',
);

content = content.replace(
    /\(\(match\.page as any\)\.text\?\.toString\?\.\(\) \?\?\s*String\(\(match\.page as any\)\.text \?\? ""\)\) as string/g,
    '(match.page.text?.toString?.() ?? String(match.page.text ?? "")) as string',
);

content = content.replace(
    />\{\(m\.page as any\)\.text\?\.toString\?\.\(\) \?\?\s*\(m\.page as any\)\.text \?\? "",\s*options\)\}/g,
    '>{m.page.text?.toString?.() ?? m.page.text ?? "", options}',
);
content = content.replace(
    />\{\(m\.item as any\)\.text\?\.toString\?\.\(\) \?\?\s*\(m\.item as any\)\.text \?\? "",\s*options\)\}/g,
    '>{m.item.text?.toString?.() ?? m.item.text ?? "", options}',
);
