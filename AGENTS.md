...
- dprint's wasm plugins are committed to `dprint-plugins/` and referenced by relative path from both `dprint.json` and `functions/dprint.json`, so formatting works without network access to `plugins.dprint.dev`. The files are stored with **Git LFS**; a clone made without git-lfs contains pointer files only. Run `scripts/ensure-dprint-plugins.sh` (invoked automatically by `scripts/setup.sh`, `scripts/test.sh`, `scripts/check_format.sh`, and the pre-push hook) to restore them. It tries three sources in order: `git lfs pull`, then `npm pack` of each plugin's npm package, then a download from `plugins.dprint.dev`. The npm stage is what makes formatting work in sandboxes that allow `registry.npmjs.org` but block the dprint plugin CDN; it only downloads tarballs and never installs into the repository. If a non-LFS stage is used in a clone where Git LFS is unusable, the script marks the files `skip-worktree` so the binaries are never staged as ordinary Git blobs (undo with `git update-index --no-skip-worktree dprint-plugins/*.wasm`). CI workflows that run dprint check out with `lfs: true`, and `gitlab-sync.yml` mirrors the LFS objects with `git lfs push --all`. To upgrade a plugin, download the new `.wasm` into `dprint-plugins/`, update `dprint.json` (and `functions/dprint.json` if it uses that plugin) plus the `PLUGIN_SOURCES` list in `scripts/ensure-dprint-plugins.sh` — keeping the pinned npm version in step with the file name so every clone gets identical bytes — and delete the old file.
