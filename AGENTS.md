...
- **Demo project**: The public `/demo` route is seeded from the template in `server/src/demo-content.ts` and must always demonstrate the current feature set. When you implement a new end-user feature, also add it to the demo template (extend an existing demo page or add a new feature page with a landing-page tour link). The reseed revision is derived automatically from the effective template. See `docs/demo-project.md` for the full policy.
...
- The dprint CLI and its wasm plugins are **exact-pinned devDependencies of the repository root**, and `dprint.json` / `functions/dprint.json` reference the plugins straight out of `node_modules/` (`./node_modules/@dprint/typescript/plugin.wasm`, and `../node_modules/...` from `functions/`). `package-lock.json` records an integrity hash for every one, so `npm ci` gives each clone identical bytes and nothing is downloaded from `plugins.dprint.dev`. Plugin paths in `dprint.json` are resolved relative to the config file, so `npx dprint` works from any subdirectory.
- **To upgrade a plugin or the CLI, run `npm install --save-dev --save-exact <package>@<version>` in the repository root — that is the whole change.** The version does not appear in any path, so no config file needs editing. Never hand-edit `package.json` or the lockfile.
- `scripts/ensure-dprint-plugins.sh` (invoked automatically by `scripts/setup.sh`, `scripts/test.sh`, `scripts/check_format.sh`, and the pre-push hook) compares each installed package's version against the pin in `package.json` and installs the root dependencies when they disagree. It checks versions rather than mere file presence because `node_modules/<package>/plugin.wasm` carries no version, so a branch switch or a plugin bump leaves the old bytes at exactly the path the config points at. It uses `npm install` when `node_modules` already exists — `npm ci` deletes the tree before fetching, and a failed install would take Playwright, pm2 and vitepress with it — and `npm ci` only for a fresh tree. Set `SKIP_DPRINT=1` to skip formatting entirely. Workflows that run dprint (`ci-format.yml`, `ci-eslint.yml`) install the root dependencies for the same reason.
- These files were previously committed under `dprint-plugins/` via **Git LFS** to survive a blocked `plugins.dprint.dev`. That was removed: the dprint CLI already came from npm, so a clone without npm access could not format either way, and the LFS setup cost a `lfs: true` in five workflows, LFS mirroring in `gitlab-sync.yml`, a `skip-worktree` workaround, and four files to keep in version lockstep. Do not reintroduce vendored plugin binaries.
