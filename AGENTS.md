...
- dprint's wasm plugins are committed to `dprint-plugins/` and referenced by relative path in `dprint.json`, so formatting works without network access to `plugins.dprint.dev`. The files are stored with **Git LFS**; a clone made without git-lfs contains pointer files only. Run `scripts/ensure-dprint-plugins.sh` (invoked automatically by `scripts/setup.sh`, `scripts/test.sh`, `scripts/check_format.sh`, and the pre-push hook) to restore them via `git lfs pull`, falling back to a download. If that fallback is used in a clone where Git LFS is unusable, the script marks the files `skip-worktree` so the binaries are never staged as ordinary Git blobs (undo with `git update-index --no-skip-worktree dprint-plugins/*.wasm`). CI workflows that run dprint check out with `lfs: true`, and `gitlab-sync.yml` mirrors the LFS objects with `git lfs push --all`. To upgrade a plugin, download the new `.wasm` into `dprint-plugins/`, update `dprint.json` and the URL list in `scripts/ensure-dprint-plugins.sh`, and delete the old file.
