...
filePaths.forEach(filePath => {
    ...
    if (content.includes("{#if showConfirmDelete}")) {
        content = content.replace(
            /\{#if showConfirmDelete\}\s*<ConfirmDialog([\s\S]*?)<\/ConfirmDialog>\s*\{\/if\}/g,
            "<ConfirmDialog$1</ConfirmDialog>",
        );
    ...
    if (content.includes("{#if showConfirmDialog}")) {
        content = content.replace(
            /\{#if showConfirmDialog\}\s*<ConfirmDialog([\s\S]*?)<\/ConfirmDialog>\s*\{\/if\}/g,
            "<ConfirmDialog$1</ConfirmDialog>",
        );
        content = content.replace(
