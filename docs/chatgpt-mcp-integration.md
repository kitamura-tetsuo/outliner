...
All successful results are JSON in the MCP text result. All tools require `outliner.read`; only the five write tools (`write_relation`, `set_view_query`, `update_grid_query`, `update_table_schema`, `update_table_records`) also require `outliner.write`.
