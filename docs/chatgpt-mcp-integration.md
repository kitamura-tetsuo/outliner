...
| `update_table_records`     | Table ID, expected revision, bounded `changes` array of `{recordId, values}`, retry fields | Before/after revisions and per-record confirmation for the whole atomic batch            | **write; non-destructive, idempotent configuration mutation** |
