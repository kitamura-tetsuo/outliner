...
source project guid and source table id, which is what lets a second paste offer
the existing copy instead of silently making another (§9.3) and what lets the
destination project ever answer where one of its tables originated.

**What is left behind stays behind.** Schedule rules targeting a cloned table do
not travel (§9.2), and a table whose last view was cut away is not deleted
(§9.4). Both are reported rather than inferred.
...
- you already pasted `sales` into this project — paste again to make a second
  copy, or reuse the existing one
...
and an uncopied schedule rule leaves a table that is correct today and frozen
tomorrow. Each is correct behavior, and each is indefensible in silence.
...
**9.2 Schedule rules do not travel, and their absence is reported by name.** A
schedule rule is `{ targetTableId, sql, rrule, … }` in the project's `schedules`
map, and the server scheduler writes its output into that table's Data Storage.
Clone a table whose rows are generated by a rule and the destination gets a
full, correct snapshot that then never grows again — the demo's own Recurring
Tasks page is exactly this shape, `routine_templates` feeding
`routine_occurrences` (`docs/schedule-sql-conventions.md`).

Not cloning them is the decision: a paste must never start writing to a
project's data on a timer the user did not ask for. Silence about it is the part
that would be indefensible, so the report names the rules that did not come
along, which turns an invisible freeze into a one-line instruction to recreate
them.

**9.3 A clone records its provenance, and a repeat paste offers to reuse it.**
Each cloned table stores the source project guid and source table id it was made
from. A later paste of the same clipboard into the same destination finds that
record and offers to bind to the copy that already exists rather than making a
second one — the "Existing Table" tab in `YjsTableBlock.svelte` already
establishes that interaction, and this is the same choice arriving by a
different route.

Reuse is an offer, not a rule: two snapshots of one source table taken at
different moments is a legitimate thing to want, and the second paste is where
the user says so. Provenance is worth storing on its own merits — it is the only
way a destination project can ever answer where one of its tables came from.
...
4. **§6.3, paste-time reporting.** Some of the information already exists —
   ...
   a rename, a failed group, a rebound `outline_items`, an uncopied schedule rule
   (§9.2) and a cross-project cut's abandoned source table (§9.4) each reach the
   user.
